...
import type { loadAnimaPlugins } from "../plugins/loader.js";
...
export async function startGatewaySidecars(params: {
  ...
  pluginRegistry: ReturnType<typeof loadAnimaPlugins>;
  ...
  // Start ANIMA browser control server (unless disabled via config).
  ...
  if (!isTruthyEnvValue(process.env.ANIMA_SKIP_GMAIL_WATCHER)) {
    try {
  ...
  // Tests can opt out via ANIMA_SKIP_CHANNELS (or legacy ANIMA_SKIP_PROVIDERS).
  const skipChannels =
    isTruthyEnvValue(process.env.ANIMA_SKIP_CHANNELS) ||
    isTruthyEnvValue(process.env.ANIMA_SKIP_PROVIDERS);
  if (!skipChannels) {
    try {
    ...
    params.logChannels.info(
      "skipping channel start (ANIMA_SKIP_CHANNELS=1 or ANIMA_SKIP_PROVIDERS=1)",
    );
  }
