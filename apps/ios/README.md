...
This is an **alpha** iOS app that connects to an Anima Gateway as a `role: node`.
...
3. On iOS: Anima → Settings → Gateway → paste setup code → Connect
...
cd apps/ios
xcodegen generate
xcodebuild test -project Anima.xcodeproj -scheme Anima -destination "platform=iOS Simulator,name=iPhone 17"
```

## Shared Code

- `apps/shared/AnimaKit` contains the shared transport/types used by the iOS app.
