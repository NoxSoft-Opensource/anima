...
struct IOSGatewayChatTransport: AnimaChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> AnimaChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(AnimaChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> AnimaChatHistoryPayload {
        ...
        return try JSONDecoder().decode(AnimaChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [AnimaChatAttachmentPayload]) async throws -> AnimaChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [AnimaChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(AnimaChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(AnimaGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: AnimaGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: AnimaChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: AnimaAgentEventPayload.self)
                        {
