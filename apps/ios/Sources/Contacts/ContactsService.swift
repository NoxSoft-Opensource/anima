...
final class ContactsService: ContactsServicing {
    ...
    func search(params: AnimaContactsSearchParams) async throws -> AnimaContactsSearchPayload {
        ...
        return AnimaContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: AnimaContactsAddParams) async throws -> AnimaContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return AnimaContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return AnimaContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> AnimaContactPayload {
        AnimaContactPayload(
