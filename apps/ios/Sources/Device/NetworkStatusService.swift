...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> AnimaNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> AnimaNetworkStatusPayload {
        let status: AnimaNetworkPathStatus = switch path.status {
    ...
    private static func fallbackPayload() -> AnimaNetworkStatusPayload {
        AnimaNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
