...
final class MotionService: MotionServicing {
    func activities(params: AnimaMotionActivityParams) async throws -> AnimaMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[AnimaMotionActivityEntry], Error>) in
        ...
        return AnimaMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: AnimaPedometerParams) async throws -> AnimaPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<AnimaPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = AnimaPedometerPayload(
