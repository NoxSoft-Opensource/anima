...
final class RemindersService: RemindersServicing {
    func list(params: AnimaRemindersListParams) async throws -> AnimaRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[AnimaReminderPayload], Error>) in
        ...
        return AnimaRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: AnimaRemindersAddParams) async throws -> AnimaRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = AnimaReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return AnimaRemindersAddPayload(reminder: payload)
    }
