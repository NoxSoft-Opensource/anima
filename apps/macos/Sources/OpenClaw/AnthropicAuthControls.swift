...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: AnimaOAuthStore.AnthropicOAuthStatus = AnimaOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: AnimaOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = AnimaOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = AnimaOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try AnimaOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Anima can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: AnimaOAuthStore.AnthropicOAuthStatus,
