...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: AnimaProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "net.noxsoft.anima", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: AnimaKit.AnyCodable]()) {
                $0[$1.key] = AnimaKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: AnimaProtocol.AnyCodable?) -> [String: AnimaProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: AnimaProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: AnimaKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: AnimaProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("anima.control.heartbeat")
    static let controlAgentEvent = Notification.Name("anima.control.agent")
}
