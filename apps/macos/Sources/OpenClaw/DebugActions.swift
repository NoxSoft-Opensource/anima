...
enum DebugActions {
    private static let verboseDefaultsKey = "anima.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "anima.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = AnimaPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Anima", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = AnimaPaths.configURL
        guard
