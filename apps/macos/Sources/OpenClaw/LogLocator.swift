...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["ANIMA_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/anima")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("anima-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("anima-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/anima/ (rolling or stdout), or nil if none exist.
