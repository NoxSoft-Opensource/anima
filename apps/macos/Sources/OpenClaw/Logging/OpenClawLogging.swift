...
enum AnimaLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = AnimaOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = AnimaFileLogHandler(label: label)
    ...
    static func parseLabel(_ label: String) -> (String, String) {
        guard let range = label.range(of: labelSeparator) else {
            return ("net.noxsoft.anima", label)
        }
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        AnimaLogging.bootstrapIfNeeded()
        let label = AnimaLogging.makeLabel(subsystem: subsystem, category: category)
...
struct AnimaOSLogHandler: LogHandler {
...
struct AnimaFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = AnimaLogging.parseLabel(self.label)
