...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "net.noxsoft.anima", category: "mac-node")
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: AnimaNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [AnimaCapability.canvas.rawValue, AnimaCapability.screen.rawValue]
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(AnimaCapability.camera.rawValue)
        }
        let rawLocationMode = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        if AnimaLocationMode(rawValue: rawLocationMode) != .off {
            caps.append(AnimaCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            AnimaSystemCommand.which.rawValue,
            AnimaSystemCommand.run.rawValue,
            AnimaSystemCommand.execApprovalsGet.rawValue,
            AnimaSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(AnimaCapability.camera.rawValue) {
        ...
        if capsSet.contains(AnimaCapability.location.rawValue) {
