...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "net.noxsoft.anima", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: AnimaPeekabooBridgeServices?
    private static var animaSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("Anima", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = AnimaPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.animaSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.animaSocketPath, privacy: .public)")
    }
...
@MainActor
private final class AnimaPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "net.noxsoft.anima.peekaboo")
