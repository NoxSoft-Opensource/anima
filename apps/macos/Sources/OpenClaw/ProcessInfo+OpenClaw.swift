...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`net.noxsoft.anima.mac`) even if the shipped
    ...
    {
        if environment["ANIMA_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "anima.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "anima.nixMode") { return true }
