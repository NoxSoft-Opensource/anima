...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "net.noxsoft.anima", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous Anima run already has an SSH listener on the expected port (common after restarts),
