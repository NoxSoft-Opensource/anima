...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "net.noxsoft.anima", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> AnimaSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: AnimaSessionPreviewEntry,
    ...
    private static func cache(payload: AnimaSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: AnimaChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [AnimaKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [AnimaKit.AnyCodable]) -> [AnimaChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(AnimaChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: AnimaChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: AnimaChatMessage) -> Bool {
    ...
    private static func toolNames(for message: AnimaChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: AnimaChatMessage) -> String? {
