...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.anima/anima.json. Prefer ANIMA_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.anima/anima.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.anima/anima.json. Restart the gateway to apply."
        }
