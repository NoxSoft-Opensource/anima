...
private let webChatSwiftLogger = Logger(subsystem: "net.noxsoft.anima", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: AnimaChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> AnimaChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(AnimaChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> AnimaChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                AnimaGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<AnimaChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any AnimaChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = AnimaChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: AnimaChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<AnimaChatView>) -> NSViewController
    {
