...
struct AnimaMacCLI {
    static func main() async {
        ...
        default:
            fputs("anima-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      anima-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      anima-mac discover [--timeout <ms>] [--json] [--include-local]
      anima-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      anima-mac connect
      anima-mac connect --url ws://127.0.0.1:18789 --json
      anima-mac discover --timeout 3000 --json
      anima-mac wizard --mode local
    """)
}
