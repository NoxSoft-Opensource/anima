...
private let chatUILogger = Logger(subsystem: "net.noxsoft.anima", category: "AnimaChatUI")

@MainActor
@Observable
public final class AnimaChatViewModel {
    public private(set) var messages: [AnimaChatMessage] = []
    ...
    public var attachments: [AnimaPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [AnimaChatPendingToolCall] = []
    public private(set) var sessions: [AnimaChatSessionEntry] = []
    private let transport: any AnimaChatTransport
    ...
    private var pendingToolCallsById: [String: AnimaChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any AnimaChatTransport) {
    ...
    public var sessionChoices: [AnimaChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [AnimaChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [AnimaChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: AnimaPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [AnimaChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: AnimaChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [AnimaChatMessage]) -> [AnimaChatMessage] {
        var result: [AnimaChatMessage] = []
    ...
    private static func dedupeKey(for message: AnimaChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [AnimaChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> AnimaChatAttachmentPayload in
            AnimaChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> AnimaChatSessionEntry {
        AnimaChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: AnimaChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: AnimaChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: AnimaAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = AnimaChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> AnimaPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
