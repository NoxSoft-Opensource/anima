...
@MainActor
struct AnimaChatComposer: View {
    @Bindable var viewModel: AnimaChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(AnimaChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(AnimaChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \AnimaPendingAttachment.id)
                { (att: AnimaPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            AnimaPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(AnimaChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(AnimaChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(AnimaChatTheme.subtleCard)
        .clipShape(Capsule())
    }
