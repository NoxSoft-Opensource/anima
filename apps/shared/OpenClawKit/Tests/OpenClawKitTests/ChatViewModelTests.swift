...
private final class TestChatTransport: @unchecked Sendable, AnimaChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [AnimaChatHistoryPayload]
    private let sessionsResponses: [AnimaChatSessionsListResponse]

    private let stream: AsyncStream<AnimaChatTransportEvent>
    private let continuation: AsyncStream<AnimaChatTransportEvent>.Continuation

    init(
        historyResponses: [AnimaChatHistoryPayload],
        sessionsResponses: [AnimaChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<AnimaChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> AnimaChatHistoryPayload {
        ...
        return self.historyResponses.last ?? AnimaChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [AnimaChatAttachmentPayload]) async throws -> AnimaChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return AnimaChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> AnimaChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? AnimaChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: AnimaChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = AnimaChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = AnimaChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { AnimaChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = AnimaChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = AnimaChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = AnimaChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { AnimaChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
