...
This document describes how the Anima ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`anima acp` exposes an ACP agent over stdio and forwards prompts to a running
Anima Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Anima Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `anima acp` over stdio.

Example config:

```bash
anima config set gateway.remote.url wss://gateway-host:18789
anima config set gateway.remote.token <token>
```

Example run:

```bash
anima acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
anima acp --session agent:main:main
anima acp --session agent:design:main
anima acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Anima ACP” to start a thread.

## Execution Model

- ACP client spawns `anima acp` and speaks ACP messages over stdio.
...
```bash
anima acp --session agent:main:main
anima acp --session-label "support inbox"
anima acp --reset-session
```
...
`anima acp` resolves the Gateway URL and auth from CLI flags or config:
