...
- Jobs persist under `~/.anima/cron/` so restarts don’t lose schedules.
...
anima cron list
anima cron run <job-id>
anima cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.anima/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `anima cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, Anima defaults to `announce`.
...
- Job store: `~/.anima/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.anima/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
anima cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
anima cron edit <jobId> --agent ops
anima cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
anima cron run <jobId>
anima cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
anima cron edit <jobId> \
...
```bash
anima system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`anima system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `ANIMA_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Anima applies exponential retry backoff for recurring jobs after consecutive errors:
