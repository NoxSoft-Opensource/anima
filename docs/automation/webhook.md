...
read_when:
  ...
  - Wiring external systems into Anima
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Anima config directory (typically `~/.anima/hooks/transforms`).
...
- `anima webhooks gmail setup` writes `hooks.gmail` config for `anima webhooks gmail run`.
