...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Anima to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
anima plugins install @anima/feishu
```

Local checkout (when running from a git repo):

```bash
anima plugins install ./extensions/feishu
```
...
If you just installed Anima, run the wizard:
...
2. Configuring app credentials in Anima
...
1. You already ran `anima channels add` for Feishu
2. The gateway is running (`anima gateway status`)
...
```bash
anima pairing approve feishu <CODE>
```
...
2. Run `anima logs --follow` and look for `chat_id`
...
2. Run `anima logs --follow` and look for `open_id`
...
| `anima gateway restart` | Restart gateway service       |
| `anima logs --follow`   | Tail gateway logs             |
...
4. Check logs: `anima logs --follow`
...
5. Ensure the gateway is running: `anima gateway status`
6. Check logs: `anima logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.anima/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.anima/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
