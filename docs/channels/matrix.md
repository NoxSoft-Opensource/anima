...
Matrix is an open, decentralized messaging protocol. Anima connects as a Matrix **user**
...
```bash
anima plugins install @anima/matrix
```

Local checkout (when running from a git repo):

```bash
anima plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `anima plugins install @anima/matrix`
   - From a local checkout: `anima plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Anima calls the same
     login endpoint, stores the access token in `~/.anima/credentials/matrix/credentials.json`,
...
- On first connection, Anima requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Anima logs a warning.
...
`~/.anima/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `anima pairing list matrix`
  - `anima pairing approve matrix <CODE>`
...
- On startup, Anima resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
