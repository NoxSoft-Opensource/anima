...
```bash
anima plugins install @anima/msteams
```

Local checkout (when running from a git repo):

```bash
anima plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
Anima will offer the local install path automatically.
...
3. Configure Anima with those credentials.
...
- Talk to Anima via Teams DMs, group chats, or channels.
...
- On startup, Anima resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.anima/anima.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Anima, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `anima-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `anima plugins install @anima/msteams`
   - From a local checkout: `anima plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "Anima in Teams", "full": "Anima in Teams" },
...
Anima handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Anima only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/AnimaShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Anima sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `anima message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.anima/msteams-polls.json`.
...
```bash
anima message send --channel msteams \
...
anima message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
anima message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
anima message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
anima message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
