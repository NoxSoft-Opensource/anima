...
```bash
anima plugins install @anima/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
anima plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Anima" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `anima pairing list nextcloud-talk`
  - `anima pairing approve nextcloud-talk <CODE>`
