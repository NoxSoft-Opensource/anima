...
read_when:
  - You want Anima to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Anima to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`anima onboard`) and `anima channels add` list optional channel plugins.
...
```bash
anima plugins install @anima/nostr
```

Use a local checkout (dev workflows):

```bash
anima plugins install --link <path-to-anima>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "anima@example.com",
        "lud16": "anima@example.com"
      }
    }
  }
}
