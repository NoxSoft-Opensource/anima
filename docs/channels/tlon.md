...
Tlon is a decentralized messenger built on Urbit. Anima connects to your Urbit ship and can
...
```bash
anima plugins install @anima/tlon
```

Local checkout (when running from a git repo):

```bash
anima plugins install ./extensions/tlon
```
...
By default, Anima blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `anima message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Anima replies in-thread.
