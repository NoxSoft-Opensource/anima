...
read_when:
  - Setting up Twitch chat integration for Anima
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Anima connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
anima plugins install @anima/twitch
```

Local checkout (when running from a git repo):

```bash
anima plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `ANIMA_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "anima", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use ANIMA_TWITCH_ACCESS_TOKEN env var)
...
```bash
ANIMA_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
