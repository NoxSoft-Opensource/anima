...
- `/usage cost` in chats: local cost summary aggregated from Anima session logs.
- CLI: `anima status --usage` prints a full per-provider breakdown.
- CLI: `anima channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
