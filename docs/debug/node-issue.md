...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Anima
...
Running Anima via Node with `tsx` fails at startup with:

```
[anima] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `anima status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch anima.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node anima.mjs status` works on Node 25.
