...
read_when:
  - Designing workspace memory (~/.anima/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Anima integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.anima/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.anima/workspace` as canonical human-readable memory.
...
```
~/.anima/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Anima**, but keep a separable core library.

### Why integrate into Anima?
...
- You want the agent itself to call the tools:
  - `anima memory recall "…" --k 25 --since 30d`
  - `anima memory reflect --since 7d`
...
Pragmatic take for `~/.anima/workspace`:
