...
description: "Complete field-by-field reference for ~/.anima/anima.json"
---

# Configuration Reference

Every field available in `~/.anima/anima.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Anima uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `anima doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["anima-dm"] },
      guilds: {
        "123456789012345678": {
...
Mattermost ships as a plugin: `anima plugins install @anima/mattermost`.
...
Anima spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@anima"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `anima.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, Anima auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Anima assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.anima/sandboxes`
    ...
    list: [
      {
        ...
        agentDir: "~/.anima/agents/main/agent",
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:anima`   | All built-in tools (excludes provider plugins)                                           |
...
Anima uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.anima/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `ANIMA_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `anima onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `anima onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `anima onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `anima onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `anima onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `anima onboard --auth-choice minimax-api`.
...
- Loaded from `~/.anima/extensions`, `<workspace>/.anima/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or ANIMA_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `ANIMA_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.anima-dev` + port `19001`), `--profile <name>` (uses `~/.anima-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.anima/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-anima-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `ANIMA_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or ANIMA_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__anima__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__anima__/a2ui/`
...
- Also serves A2UI at `/__anima__/a2ui/`.
...
- Hostname defaults to `anima`. Override with `ANIMA_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.anima/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `anima dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.anima/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${ANIMA_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.anima/credentials/oauth.json`.
...
- Default log file: `/tmp/anima/anima-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `anima doctor --fix` can strip unknown keys).
