...
- During heartbeat runs, Anima treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `anima sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Anima skips the heartbeat run entirely (no model call).
...
```bash
anima system event --text "Check for urgent follow-ups" --mode now
```
