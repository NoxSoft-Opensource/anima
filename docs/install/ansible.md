---
summary: "Automated, hardened Anima installation with Ansible, Tailscale VPN, and firewall isolation"
read_when:
...
5. **Anima** (host-based, not containerized)
...
After installation completes, switch to the anima user:
...
1. **Onboarding wizard**: Configure Anima settings
...
# Provider login (run as anima user)
sudo -i -u anima
...
# Or run directly (then manually execute /tmp/anima-setup.sh after)
# ansible-playbook playbook.yml --ask-become-pass
```

## Updating Anima

The Ansible installer sets up Anima for manual updates. See [Updating](/install/updating) for the standard update flow.
...
Make sure you're running as the `anima` user:
