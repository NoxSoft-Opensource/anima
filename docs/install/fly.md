---
title: Fly.io
description: Deploy Anima on Fly.io
---

# Fly.io Deployment

**Goal:** Anima Gateway running on a [Fly.io](https://fly.io) machine with persistent storage, automatic HTTPS, and Discord/channel access.
...
| `internal_port = 3000`      | Must match `--port 3000` (or `ANIMA_GATEWAY_PORT`) for Fly health checks |
| `memory = "2048mb"`         | 512MB is too small; 2GB recommended                                      |
| `ANIMA_STATE_DIR = "/data"` | Persists state on the volume                                             |

## 3) Set secrets

```bash
# Required: Gateway token (for non-loopback binding)
fly secrets set ANIMA_GATEWAY_TOKEN=$(openssl rand -hex 32)
...
- Non-loopback binds (`--bind lan`) require `ANIMA_GATEWAY_TOKEN` for security.
- Treat these tokens like passwords.
- **Prefer env vars over config file** for all API keys and tokens. This keeps secrets out of `anima.json` where they could be accidentally exposed or logged.
...
**Note:** With `ANIMA_STATE_DIR=/data`, the config path is `/data/anima.json`.
...
Or visit `https://my-anima.fly.dev/`

Paste your gateway token (the one from `ANIMA_GATEWAY_TOKEN`) to authenticate.
...
**Fix:** Ensure `internal_port` matches the gateway port (set `--port 3000` or `ANIMA_GATEWAY_PORT=3000`).
...
If using `--allow-unconfigured`, the gateway creates a minimal config. Your custom config at `/data/anima.json` should be read on restart.

Verify the config exists:

```bash
fly ssh console --command "cat /data/anima.json"
```
...
echo '{"your":"config"}' | fly ssh console -C "tee /data/anima.json"

# Or use sftp
fly sftp shell
> put /local/path/config.json /data/anima.json
```

**Note:** `fly sftp` may fail if the file already exists. Delete first:

```bash
fly ssh console --command "rm /data/anima.json"
```
...
**Fix:** Ensure `ANIMA_STATE_DIR=/data` is set in `fly.toml` and redeploy.
...
```bash
# List current IPs
fly ips list -a my-anima

# Release public IPs
fly ips release <public-ipv4> -a my-anima
fly ips release <public-ipv6> -a my-anima
...
fly ips allocate-v6 --private -a my-anima
```
...
fly proxy 3000:3000 -a my-anima
