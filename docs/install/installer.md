...
<Note>
If install succeeds but `anima` is not found in a new terminal, see [Node.js troubleshooting](/install/node#troubleshooting).
</Note>
...
<Steps>
  ...
  <Step title="Install Anima">
    ...
    - `git` method: clone/update repo, install deps with pnpm, build, then install wrapper at `~/.local/bin/anima`
  </Step>
  <Step title="Post-install tasks">
    - Runs `anima doctor --non-interactive` on upgrades and git installs (best effort)
...
If run inside an Anima checkout (`package.json` + `pnpm-workspace.yaml`), the script offers:
...
| `ANIMA_GIT_UPDATE=0\|1`                  | Toggle git updates                            |
| `ANIMA_NO_PROMPT=1`                      | Disable prompts                               |
...
<Info>
Designed for environments where you want everything under a local prefix (default `~/.anima`) and no system Node dependency.
</Info>
...
<Steps>
  ...
  <Step title="Install Anima under prefix">
    Installs with npm using `--prefix <prefix>`, then writes wrapper to `<prefix>/bin/anima`.
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="Install Anima">
    ...
    - `git` method: clone/update repo, install/build with pnpm, and install wrapper at `%USERPROFILE%\.local\bin\anima.cmd`
  </Step>
  <Step title="Post-install tasks">
    Adds needed bin directory to user PATH when possible, then runs `anima doctor --non-interactive` on upgrades and git installs (best effort).
  </Step>
</Steps>
...
| `ANIMA_GIT_DIR=<path>`          | Checkout directory |
| `ANIMA_NO_ONBOARD=1`            | Skip onboarding    |
| `ANIMA_GIT_UPDATE=0`            | Disable git pull   |
| `ANIMA_DRY_RUN=1`               | Dry run mode       |
...
<AccordionGroup>
  ...
  <Accordion title='Windows: "anima is not recognized"'>
  ...
  <Accordion title="anima not found after install">
