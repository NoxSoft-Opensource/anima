...
Deploy Anima on Railway with a one-click template and finish setup in your browser.
...
<a href="https://railway.com/deploy/anima-railway-template" target="_blank" rel="noreferrer">
  Deploy on Railway
</a>
...
- `https://<your-railway-domain>/anima` — Control UI

## What you get

- Hosted Anima Gateway + Control UI
...
- `ANIMA_STATE_DIR=/data/.anima` (recommended)
- `ANIMA_WORKSPACE_DIR=/data/workspace` (recommended)
- `ANIMA_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your Anima state + workspace so you can migrate to another host without losing config or memory.
