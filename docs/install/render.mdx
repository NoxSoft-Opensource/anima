...
Deploy Anima on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://gitlab.com/sylys-group/anima)
...
repository configures everything needed to run Anima:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: ANIMA_STATE_DIR
        value: /data/.anima
      - key: ANIMA_WORKSPACE_DIR
        value: /data/workspace
      - key: ANIMA_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/anima`.
...
If you use the original Anima repository, Render will not auto-deploy your Anima. To update it, run a manual Blueprint sync from the dashboard.
...
For Anima, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Anima host.
