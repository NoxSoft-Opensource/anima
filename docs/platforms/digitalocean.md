---
summary: "Anima on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up Anima on DigitalOcean
  - Looking for cheap VPS hosting for Anima
title: "DigitalOcean"
---

# Anima on DigitalOcean

## Goal

Run a persistent Anima Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status anima-gateway.service

# View logs
journalctl --user -u anima-gateway.service -f
```
...
```bash
anima config set gateway.bind tailnet
anima gateway restart
```
...
- `~/.anima/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u anima --no-pager -n 50
```
