...
service via the external `anima` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`ANIMA_PROFILE`; legacy `com.anima.*` is supported).
