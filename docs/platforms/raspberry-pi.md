---
summary: "Anima on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Anima on a Raspberry Pi
  - Running Anima on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Anima on Raspberry Pi

## Goal

Run a persistent, always-on Anima Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Anima features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled anima
...
```bash
# Check logs
journalctl -u anima --no-pager -n 100

# Common fix: rebuild
cd ~/anima  # if using hackable install
npm run build
sudo systemctl restart anima
```
