...
read_when:
  - You want to place an outbound voice call from Anima
...
Voice calls for Anima via a plugin. Supports outbound notifications and
...
- Use `anima voicecall ...` or the `voice_call` tool
...
```bash
anima plugins install ./extensions/voice-call
