---
summary: "Use MiniMax M2.1 in Anima"
read_when:
  - You want MiniMax models in Anima
...
```bash
anima plugins enable minimax-portal-auth  # skip if already loaded.
anima gateway restart  # restart if gateway is already running
anima onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://gitlab.com/sylys-group/anima/-/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `anima.json`:
...
- Use `anima models list` and `anima models set minimax/MiniMax-M2.1` to switch.
...
- Running `anima configure` and selecting **MiniMax M2.1**, or
