---
summary: "Use Venice AI privacy-focused models in Anima"
read_when:
  - You want privacy-focused inference in Anima
...
```bash
anima onboard --auth-choice venice-api-key
```
...
```bash
anima chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Anima shows all available Venice models. Pick based on your needs:
...
```bash
anima models set venice/claude-opus-45
anima models set venice/llama-3.3-70b
```

List all available models:

```bash
anima models list | grep venice
```

## Configure via `anima configure`

1. Run `anima configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Anima automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
anima chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
anima chat --model venice/claude-opus-45

# Use uncensored model
anima chat --model venice/venice-uncensored

# Use vision model with image
anima chat --model venice/qwen3-vl-235b-a22b

# Use coding model
anima chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `anima models list` to see currently available models. Some models may be temporarily offline.
