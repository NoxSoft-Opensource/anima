...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.anima/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.anima/node.json` (node id + pairing token).
...
Path: `~/.anima/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.anima/exec-approvals.sock",
...
- Unix socket at `~/.anima/exec-approvals.sock` (0600).
