...
- Publish as `anima/plugin-sdk` (or export from core under `anima/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `AnimaPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: AnimaConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: AnimaConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: AnimaConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: AnimaConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: AnimaConfig): string;
  };
};
```
...
- Introduce `anima/plugin-sdk`.
- Add `api.runtime` to `AnimaPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `animaRuntime: ">=2026.2.0"`).
