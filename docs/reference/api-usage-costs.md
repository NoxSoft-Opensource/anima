...
Anima features that can generate provider usage or paid API calls.
...
- `anima status --usage` and `anima channels list` show provider **usage windows**
...
Anima can pick up credentials from:
...
`anima models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
