...
Anima integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Anima owns lifecycle when `channels.signal.autoStart=true`.
...
- Anima spawns `imsg rpc` as a child process (legacy iMessage integration).
