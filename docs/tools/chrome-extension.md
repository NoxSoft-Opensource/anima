---
summary: "Chrome extension: let Anima drive your existing Chrome tab"
read_when:
...
The Anima Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate anima-managed Chrome profile.
...
Anima then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Anima release (npm package) as static files. There is no separate “build” step.

After upgrading Anima:

- Re-run `anima browser extension install` to refresh the installed files under your Anima state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Anima ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Anima to control.
...
- `ON`: attached; Anima can drive that tab.
...
Debugging: `anima sandbox explain`
...
`anima browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `anima browser extension install` first to copy the extension to a stable location under your Anima state directory.
...
- **This is not isolated** like the dedicated anima-managed profile.
