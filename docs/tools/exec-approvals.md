...
- **gateway host** → `anima` process on the gateway machine
...
`~/.anima/exec-approvals.json` directly.

CLI: `anima approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
