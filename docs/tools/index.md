---
summary: "Agent tool surface for Anima (browser, canvas, nodes, message, cron) replacing legacy `anima-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `anima-*` skills
title: "Tools"
---

# Tools (Anima)

Anima exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `anima-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `anima.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Anima logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:anima`: all built-in Anima tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`anima node run`).
...
- Requires a Brave API key (recommended: `anima configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated Anima-managed browser.
...
- Quick smoke: `anima nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `anima gateway` restart in-place)
