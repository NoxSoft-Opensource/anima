...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `anima sandbox explain`.
...
Legacy `agent.*` configs are migrated by `anima doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=anima-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${ANIMA_STATE_DIR:-$HOME/.anima}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
