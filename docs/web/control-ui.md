...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/anima`)
...
If the page fails to load, start the Gateway first: `anima gateway`.
...
you revoke it with `anima devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.anima/anima.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Anima
...
Anima **blocks** Control UI connections without device identity.
