...
```bash
anima tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `anima tui --deliver`
...
- Check the Gateway logs: `anima logs --follow`.
- Confirm the agent can run: `anima status` and `anima models status`.
...
- No agents in picker: check `anima agents list` and your routing config.
