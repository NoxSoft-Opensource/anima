...
- **会话存储**（聊天历史 + 路由状态）位于 `~/.anima/agents/<agentId>/sessions` 下。
...
Skills 通过每个工作区的 `skills/` 文件夹实现每智能体独立，共享的 Skills 可从 `~/.anima/skills` 获取。参见 [Skills：每智能体 vs 共享](/tools/skills#per-agent-vs-shared-skills)。
...
- 状态目录：`~/.anima`（或 `ANIMA_STATE_DIR`）
- 工作区：`~/.anima/workspace`（或 `~/.anima/workspace-<agentId>`）
- 智能体目录：`~/.anima/agents/<agentId>/agent`（或 `agents.list[].agentDir`）
- 会话：`~/.anima/agents/<agentId>/sessions`

### 单智能体模式（默认）

如果你什么都不做，Anima 运行单个智能体：

- `agentId` 默认为 **`main`**。
- 会话键为 `agent:main:<mainKey>`。
- 工作区默认为 `~/.anima/workspace`（或当设置了 `ANIMA_PROFILE` 时为 `~/.anima/workspace-<profile>`）。
- 状态默认为 `~/.anima/agents/main/agent`。
...
```js
{
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // 可选覆盖。默认：~/.anima/credentials/whatsapp/personal
