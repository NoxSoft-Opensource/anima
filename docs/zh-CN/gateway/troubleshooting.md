...
| `anima gateway probe`           | Gateway 网关发现 + 可达性（本地 + 远程目标）                                          | 当你怀疑正在探测错误的 Gateway 网关时 |
| `anima channels status --probe` | 向运行中的 Gateway 网关查询渠道状态（并可选探测）                                     | 当 Gateway 网关可达但渠道异常时       |
| `anima gateway status`          | 监管程序状态（launchd/systemd/schtasks）、运行时 PID/退出、最后的 Gateway 网关错误    | 当服务"看起来已加载"但没有运行时      |
| `anima logs --follow`           | 实时日志（运行时问题的最佳信号）                                                      | 当你需要实际的故障原因时              |

**分享输出：** 优先使用 `anima status --all`（它会隐藏令牌）。如果你粘贴 `anima status`，考虑先设置 `ANIMA_SHOW_SECRETS=0`（令牌预览）。
...
- 或在 **Gateway 网关主机**上粘贴 setup-token：
  ```bash
  anima models auth setup-token --provider anthropic
  ```
...
```bash
anima models auth paste-token --provider anthropic
anima models status
```
...
- 优先：`anima logs --follow`
- 文件日志（始终）：`/tmp/anima/anima-YYYY-MM-DD.log`（或你配置的 `logging.file`）
- macOS LaunchAgent（如果已安装）：`$ANIMA_STATE_DIR/logs/gateway.log` 和 `gateway.err.log`
- Linux systemd（如果已安装）：`journalctl --user -u anima-gateway[-<profile>].service -n 200 --no-pager`
...
- 或直接设置：
  ```bash
  anima config set gateway.mode local
  ```
...
- 设置远程 URL 并保持 `gateway.mode=remote`：
  ```bash
  anima config set gateway.mode remote
  anima config set gateway.remote.url "wss://gateway.example.com"
  ```

**仅临时/开发使用：** 传递 `--allow-unconfigured` 以在没有
`gateway.mode=local` 的情况下启动 Gateway 网关。

**还没有配置文件？** 运行 `anima setup` 创建初始配置，然后重新运行
Gateway 网关。
...
变量如 `DISPLAY` 应该放在 `~/.anima/.env` 中（由 Gateway 网关早期加载）。
...
服务是用 Bun 或版本管理的 Node 路径安装的，运行 `anima doctor`
迁移到系统 Node 安装。
...
- 或将密钥烘焙到你的自定义沙箱镜像中
- 然后运行 `anima sandbox recreate --agent <id>`（或 `--all`）
...
- `gateway.mode` 必须为 `local` 才能运行 `anima gateway` 和服务。
- 如果你设置了 `gateway.mode=remote`，**CLI 默认**使用远程 URL。服务可能仍在本地运行，但你的 CLI 可能在探测错误的位置。使用 `anima gateway status` 查看服务解析的端口 + 探测目标（或传递 `--url`）。
- `anima gateway status` 和 `anima doctor` 在服务看起来正在运行但端口关闭时会显示日志中的**最后 Gateway 网关错误**。
- 非本地回环绑定（`lan`/`tailnet`/`custom`，或本地回环不可用时的 `auto`）需要认证：
  `gateway.auth.token`（或 `ANIMA_GATEWAY_TOKEN`）。
- `gateway.remote.token` 仅用于远程 CLI 调用；它**不**启用本地认证。
- `gateway.token` 被忽略；使用 `gateway.auth.token`。

**如果 `anima gateway status` 显示配置不匹配**

- `Config (cli): ...` 和 `Config (service): ...` 通常应该匹配。
- 如果不匹配，你几乎肯定是在编辑一个配置而服务运行的是另一个。
- 修复：从你希望服务使用的相同 `--profile` / `ANIMA_STATE_DIR` 重新运行 `anima gateway install --force`。

**如果 `anima gateway status` 报告服务配置问题**

- 监管程序配置（launchd/systemd/schtasks）缺少当前默认值。
- 修复：运行 `anima doctor` 更新它（或 `anima gateway install --force` 完全重写）。
...
- 修复：设置 `gateway.auth.mode` + `gateway.auth.token`（或导出 `ANIMA_GATEWAY_TOKEN`）并重启服务。

**如果 `anima gateway status` 显示 `bind=tailnet` 但未找到 tailnet 接口**
...
症状：`pwd` 或文件工具显示 `~/.anima/sandboxes/...` 即使你
期望的是主机工作区。
...
Anima 有意拒绝**较旧/不安全的模型**（尤其是那些更容易受到提示词注入攻击的模型）。如果你看到此错误，该模型名称已不再支持。

**修复：**

- 为提供商选择**最新**模型并更新你的配置或模型别名。
- 如果你不确定哪些模型可用，运行 `anima models list` 或
  `anima models scan` 并选择一个支持的模型。
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${ANIMA_CONFIG_PATH:-$HOME/.anima/anima.json}"
```
...
anima logs --follow
# 或者如果你想快速过滤：
tail -f "$(ls -t /tmp/anima/anima-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
anima logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.anima/agents/<agentId>/sessions/
```
...
# 查看最近的连接事件
anima logs --limit 200 | grep "connection\\|disconnect\\|logout"
```

**修复：** 通常在 Gateway 网关运行后会自动重连。如果卡住，重启 Gateway 网关进程（无论你如何监管它），或使用详细输出手动运行：

```bash
anima gateway --verbose
```

如果你已登出/取消关联：

```bash
anima channels logout
trash "${ANIMA_STATE_DIR:-$HOME/.anima}/credentials" # 如果 logout 无法完全清除所有内容
anima channels login --verbose       # 重新扫描二维码
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/anima/anima-*.log | head -1)" | tail -20
```
...
当配置包含未知键、格式错误的值或无效类型时，Anima 现在拒绝启动。
这是为了安全而故意设计的。
...
- `anima doctor` 报告每个无效条目。
- `anima doctor --fix` 应用迁移/修复并重写配置。
- 诊断命令如 `anima logs`、`anima health`、`anima status`、`anima gateway status` 和 `anima gateway probe` 即使配置无效也能运行。

### "All models failed" — 我应该首先检查什么？

- **凭证**存在于正在尝试的提供商（认证配置文件 + 环境变量）。
- **模型路由**：确认 `agents.defaults.model.primary` 和回退是你可以访问的模型。
- `/tmp/anima/…` 中的 **Gateway 网关日志**以获取确切的提供商错误。
- **模型状态**：使用 `/model status`（聊天）或 `anima models status`（CLI）。
...
5. 运行 `anima channels status --probe` 获取审计提示。
...
端点接受 JSON Schema 的严格子集。Anima 在当前 `main` 中清理/规范化工具
...
anima gateway status
anima gateway stop
# 或：launchctl bootout gui/$UID/bot.molt.gateway（用 bot.molt.<profile> 替换；旧版 com.anima.* 仍然有效）
```
...
**修复 3：检查 CLI 安装**
确保全局 `anima` CLI 已安装且与应用版本匹配：

```bash
anima --version
npm install -g anima@<version>
```
...
```bash
# 在配置中打开跟踪日志：
#   ${ANIMA_CONFIG_PATH:-$HOME/.anima/anima.json} -> { logging: { level: "trace" } }
#
# 然后运行详细命令将调试输出镜像到标准输出：
anima gateway --verbose
anima channels login --verbose
```
...
trash "${ANIMA_STATE_DIR:-$HOME/.anima}"
anima channels login         # 重新配对 WhatsApp
anima gateway restart           # 或：anima gateway
```
...
1. 首先检查日志：`/tmp/anima/`（默认：`anima-YYYY-MM-DD.log`，或你配置的 `logging.file`）
2. 在 GitHub 上搜索现有问题
3. 提交新问题时包含：
   - Anima 版本
   - 相关日志片段
