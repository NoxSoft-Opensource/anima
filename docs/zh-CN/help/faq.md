...
- [快速开始与首次运行设置](#quick-start-and-firstrun-setup)
  ...
  - [无法访问 docs.anima.ai（SSL 错误），怎么办？](#i-cant-access-docsanimaai-ssl-error-what-now)
...
https://github.com/anima/anima/issues
https://github.com/anima/anima/pulls
...
查看 GitHub 变更日志：
https://github.com/anima/anima/blob/main/CHANGELOG.md

最新条目在顶部。如果顶部部分标记为 **Unreleased**，则下一个带日期的部分是最新发布版本。条目按**亮点**、**变更**和**修复**分组（需要时还有文档/其他部分）。

### 无法访问 docs.anima.ai（SSL 错误），怎么办

一些 Comcast/Xfinity 连接通过 Xfinity Advanced Security 错误地拦截了 `docs.anima.ai`。禁用该功能或将 `docs.anima.ai` 加入白名单，然后重试。更多详情：[故障排除](/help/troubleshooting#docsanimaai-shows-an-ssl-error-comcastxfinity)。
请帮助我们在此处报告以解除封锁：https://spa.xfinity.com/check_url_status。

如果仍然无法访问该网站，文档在 GitHub 上有镜像：
https://github.com/anima/anima/tree/main/docs
...
可以。通过 `~/.anima/anima.json` 中的 `skills.load.extraDirs` 添加额外目录（最低优先级）。默认优先级保持不变：`<workspace>/skills` → `~/.anima/skills` → 内置 → `skills.load.extraDirs`。`animahub` 默认安装到 `./skills`，Anima 将其视为 `<workspace>/skills`。
...
使用 **AnimaHub**（CLI）或将 Skills 放入你的工作区。macOS Skills UI 在 Linux 上不可用。
浏览 Skills：https://animahub.com。

安装 AnimaHub CLI（选择一个包管理器）：
...
AnimaHub 安装到当前目录下的 `./skills`（或回退到你配置的 Anima 工作区）；Anima 在下一个会话中将其视为 `<workspace>/skills`。对于跨智能体共享的 Skills，将它们放在 `~/.anima/skills/<name>/SKILL.md`。某些 Skills 期望通过 Homebrew 安装二进制文件；在 Linux 上意味着 Linuxbrew（参阅上面的 Homebrew Linux 常见问题条目）。参阅[Skills](/tools/skills)和 [AnimaHub](/tools/animahub)。
...
仍然卡住？在 [Discord](https://discord.com/invite/clawd) 中提问或发起 [GitHub 讨论](https://github.com/anima/anima/discussions)。
