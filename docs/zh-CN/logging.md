...
```bash
anima channels logs --channel whatsapp
```
...
所有日志配置都在 `~/.anima/anima.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/anima/anima-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- Anima 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
```
ANIMA_DIAGNOSTICS=telegram.http,telegram.payload
```
...
- 你也可以使用 `anima plugins enable diagnostics-otel` 启用插件。
...
- `anima.tokens`（计数器，属性：`anima.token`、`anima.channel`、`anima.provider`、`anima.model`）
- `anima.cost.usd`（计数器，属性：`anima.channel`、`anima.provider`、`anima.model`）
- `anima.run.duration_ms`（直方图，属性：`anima.channel`、`anima.provider`、`anima.model`）
- `anima.context.tokens`（直方图，属性：`anima.context`、`anima.channel`、`anima.provider`、`anima.model`）

消息流：

- `anima.webhook.received`（计数器，属性：`anima.channel`、`anima.webhook`）
- `anima.webhook.error`（计数器，属性：`anima.channel`、`anima.webhook`）
- `anima.webhook.duration_ms`（直方图，属性：`anima.channel`、`anima.webhook`）
- `anima.message.queued`（计数器，属性：`anima.channel`、`anima.source`）
- `anima.message.processed`（计数器，属性：`anima.channel`、`anima.outcome`）
- `anima.message.duration_ms`（直方图，属性：`anima.channel`、`anima.outcome`）

队列 + 会话：

- `anima.queue.lane.enqueue`（计数器，属性：`anima.lane`）
- `anima.queue.lane.dequeue`（计数器，属性：`anima.lane`）
- `anima.queue.depth`（直方图，属性：`anima.lane` 或 `anima.channel=heartbeat`）
- `anima.queue.wait_ms`（直方图，属性：`anima.lane`）
- `anima.session.state`（计数器，属性：`anima.state`、`anima.reason`）
- `anima.session.stuck`（计数器，属性：`anima.state`）
- `anima.session.stuck_age_ms`（直方图，属性：`anima.state`）
- `anima.run.attempt`（计数器，属性：`anima.attempt`）

### 导出的 span（名称 + 关键属性）

- `anima.model.usage`
  - `anima.channel`、`anima.provider`、`anima.model`
  - `anima.sessionKey`、`anima.sessionId`
  - `anima.tokens.*`（input/output/cache_read/cache_write/total）
- `anima.webhook.processed`
  - `anima.channel`、`anima.webhook`、`anima.chatId`
- `anima.webhook.error`
  - `anima.channel`、`anima.webhook`、`anima.chatId`、`anima.error`
- `anima.message.processed`
  - `anima.channel`、`anima.outcome`、`anima.chatId`、`anima.messageId`、`anima.sessionKey`、`anima.sessionId`、`anima.reason`
- `anima.session.stuck`
  - `anima.state`、`anima.ageMs`、`anima.queueDepth`、`anima.sessionKey`、`anima.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `anima doctor`。
