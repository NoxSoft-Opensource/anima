...
- `~/Library/Application Support/Anima/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `anima-canvas://<session>/<path>`

示例：

- `anima-canvas://main/` → `<canvasRoot>/main/index.html`
- `anima-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `anima-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
anima nodes canvas present --node <id>
anima nodes canvas navigate --node <id> --url "/"
anima nodes canvas eval --node <id> --js "document.title"
anima nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__anima__/a2ui/
```
...
anima nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
anima nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "anima://agent?message=Review%20this%20design";
```
