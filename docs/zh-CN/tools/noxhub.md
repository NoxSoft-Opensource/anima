...
read_when:
  - 向新用户介绍 NoxHub
  - 安装、搜索或发布 Skills
  - 说明 NoxHub CLI 标志和同步行为
summary: NoxHub 指南：公共 Skills 注册中心 + CLI 工作流
title: NoxHub
x-i18n:
  ...
  source_path: tools/noxhub.md
  workflow: 15
---

# NoxHub

NoxHub 是 **Anima 的公共 Skills 注册中心**。它是一项免费服务：所有 Skills 都是公开的、开放的，所有人都可以查看、共享和复用。Skills 就是一个包含 `SKILL.md` 文件（以及辅助文本文件）的文件夹。你可以在网页应用中浏览 Skills，也可以使用 CLI 来搜索、安装、更新和发布 Skills。

网站：[noxhub.com](https://noxhub.com)

## 适用人群（新手友好）

如果你想为 Anima 智能体添加新功能，NoxHub 是查找和安装 Skills 的最简单方式。你不需要了解后端的工作原理。你可以：
...
默认情况下，CLI 会将 Skills 安装到当前工作目录下的 `./skills`。如果已配置 Anima 工作区，`noxhub` 会回退到该工作区，除非你通过 `--workdir`（或 `NOXHUB_WORKDIR`）进行覆盖。Anima 从 `<workspace>/skills` 加载工作区 Skills，并会在**下一个**会话中生效。如果你已经在使用 `~/.anima/skills` 或内置 Skills，工作区 Skills 优先级更高。
...
- `noxhub login`（浏览器流程）或 `noxhub login --token <token>`
- `noxhub logout`
- `noxhub whoami`
...
- `noxhub list`（读取 `.noxhub/lock.json`）
...
`noxhub sync` 首先扫描当前工作目录。如果未找到 Skills，它会回退到已知的旧版位置（例如 `~/anima/skills` 和 `~/.anima/skills`）。这样设计是为了在不需要额外标志的情况下找到旧版 Skills 安装。

### 存储和锁文件

- 已安装的 Skills 记录在工作目录下的 `.noxhub/lock.json` 中。
- 认证令牌存储在 NoxHub CLI 配置文件中（可通过 `NOXHUB_CONFIG_PATH` 覆盖）。

### 遥测（安装计数）

当你在登录状态下运行 `noxhub sync` 时，CLI 会发送一个最小快照用于计算安装次数。你可以完全禁用此功能：
...
- `NOXHUB_SITE`：覆盖网站 URL。
- `NOXHUB_REGISTRY`：覆盖注册中心 API URL。
- `NOXHUB_CONFIG_PATH`：覆盖 CLI 存储令牌/配置的位置。
- `NOXHUB_WORKDIR`：覆盖默认工作目录。
- `NOXHUB_DISABLE_TELEMETRY=1`：禁用 `sync` 的遥测功能。
