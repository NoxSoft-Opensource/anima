...
import { approveDevicePairing, listDevicePairing } from "anima/plugin-sdk";
...
function resolveGatewayPort(cfg: AnimaPluginApi["config"]): number {
  const envRaw =
    process.env.ANIMA_GATEWAY_PORT?.trim() || process.env.ANIMA_GATEWAY_PORT?.trim();
  if (envRaw) {
...
async function resolveTailnetHost(api: AnimaPluginApi): Promise<string | null> {
...
function resolveAuth(cfg: AnimaPluginApi["config"]): ResolveAuthResult {
  const mode = cfg.gateway?.auth?.mode;
  const token =
    process.env.ANIMA_GATEWAY_TOKEN?.trim() ||
    process.env.ANIMA_GATEWAY_TOKEN?.trim() ||
    cfg.gateway?.auth?.token?.trim();
  const password =
    process.env.ANIMA_GATEWAY_PASSWORD?.trim() ||
    process.env.ANIMA_GATEWAY_PASSWORD?.trim() ||
...
async function resolveGatewayUrl(api: AnimaPluginApi): Promise<ResolveUrlResult> {
