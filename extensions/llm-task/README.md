...
`anima.invoke --each`) without adding new Anima code per workflow.
...
This extension depends on Anima internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Anima extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.anima/extensions` as a standalone plugin directory.
