...
// NOTE: This extension is intended to be bundled with Anima.
// When running from source (tests/dev), Anima internals live under src/.
...
export function createLlmTaskTool(api: AnimaPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via anima.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "anima-llm-task-"));
