...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["animaPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["anima", "pollId"]) ??
    readNestedString(value, ["anima", "poll", "id"]) ??
    readNestedString(value, ["data", "animaPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "anima", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        msteams: {
          type: "messageBack",
          text: "anima poll vote",
          displayText: "Vote recorded",
          value: { animaPollId: pollId, pollId },
        },
      },
    ],
  };
