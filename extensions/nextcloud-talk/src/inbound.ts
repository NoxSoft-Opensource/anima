...
export async function handleNextcloudTalkInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as AnimaConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as AnimaConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as AnimaConfig);
