...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.ANIMA_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "anima-nostr-"));
  process.env.ANIMA_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        ...
        return path.join(homedir(), ".anima");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.ANIMA_STATE_DIR = previous;
    }
