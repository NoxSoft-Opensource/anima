...
metadata: { "anima": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Anima `sessions_spawn`
- **File I/O** == Anima `read`/`write`
- **Remote fetch** == Anima `web_fetch` (or `exec` with curl when POST is required)
