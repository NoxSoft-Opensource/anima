import type { AnimaPluginApi, AnimaPluginService } from "anima/plugin-sdk";
...
function normalizeDenyList(cfg: AnimaPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: AnimaPluginApi["config"]): string[] {
...
export default function register(api: AnimaPluginApi) {
  ...
  const timerService: AnimaPluginService = {
