/**
 * Twitch channel plugin for Anima.
 *
...
import { buildChannelConfigSchema } from "anima/plugin-sdk";
import type {
 ...
 * for Anima. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: AnimaConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: AnimaConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: AnimaConfig): boolean => {
