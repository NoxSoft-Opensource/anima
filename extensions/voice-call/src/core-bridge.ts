...
function resolveAnimaRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.ANIMA_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set ANIMA_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveAnimaRoot(), "dist", "extensionAPI.js");
