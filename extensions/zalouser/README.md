...
Anima extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
anima channels login --channel zalouser
...
```bash
anima message send --channel zalouser --target <threadId> --message "Hello from Anima!"
```
...
```bash
anima channels login --channel zalouser              # Login via QR
anima channels login --channel zalouser --account work
anima channels status --probe
anima channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
anima directory self --channel zalouser
anima directory peers list --channel zalouser --query "name"
anima directory groups list --channel zalouser --query "work"
anima directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
anima message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
anima message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `anima channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
anima channels login --channel zalouser --account work
anima message send --channel zalouser --account work --target <id> --message "Hello"
