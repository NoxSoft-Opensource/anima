...
#
# Suggested cron: */30 * * * * /home/admin/anima/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Anima notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., anima-alerts)
...
STATE_FILE="$HOME/.anima/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Anima if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use anima
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Anima to $NOTIFY_PHONE..."
            anima send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Anima is down. Run: ssh l36 '~/anima/scripts/mobile-reauth.sh'" "urgent"
    exit 1
