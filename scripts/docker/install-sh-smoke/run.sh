...
INSTALL_URL="${ANIMA_INSTALL_URL:-https://anima.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${ANIMA_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${ANIMA_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="anima"
PACKAGE_NAME="${ANIMA_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (ANIMA_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${ANIMA_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${ANIMA_INSTALL_LATEST_OUT:-}"
fi
