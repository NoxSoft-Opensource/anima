...
IMAGE_NAME="anima-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="anima-net-e2e-$$"
GW_NAME="anima-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "ANIMA_GATEWAY_TOKEN=$TOKEN" \
	  -e "ANIMA_SKIP_CHANNELS=1" \
	  -e "ANIMA_SKIP_GMAIL_WATCHER=1" \
	  -e "ANIMA_SKIP_CRON=1" \
	  -e "ANIMA_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
