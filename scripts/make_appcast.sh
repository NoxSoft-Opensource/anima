...
ZIP=${1:?"Usage: $0 Anima-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/anima/anima/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Anima-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/anima/anima/releases/download/v${VERSION}/"}
