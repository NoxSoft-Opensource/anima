...
# Build and bundle Anima into a minimal .app we can open.
# Outputs to dist/Anima.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Anima.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Anima"
BUNDLE_ID="${BUNDLE_ID:-net.noxsoft.anima.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/anima/anima/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Anima/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :AnimaBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :AnimaGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Anima"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Anima"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Anima"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Anima" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Anima/Resources/Anima.icns" "$APP_ROOT/Contents/Resources/Anima.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Anima/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying AnimaKit resources"
ANIMAKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/AnimaKit_AnimaKit.bundle"
if [ -d "$ANIMAKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/AnimaKit_AnimaKit.bundle"
  cp -R "$ANIMAKIT_BUNDLE" "$APP_ROOT/Contents/Resources/AnimaKit_AnimaKit.bundle"
else
  echo "WARN: AnimaKit resource bundle not found at $ANIMAKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Anima"
killall -q Anima 2>/dev/null || true
