#!/usr/bin/env bash
# Reset Anima like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${ANIMA_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="Anima.app/Contents/MacOS/Anima"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/Anima"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/Anima"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/Anima"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/net.noxsoft.anima.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/anima-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${ANIMA_RESTART_LOG:-/tmp/anima-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${ANIMA_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.anima/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  ANIMA_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node anima.mjs daemon install --force --runtime node"
      log "  node anima.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.anima/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/net.noxsoft.anima.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing Anima instances"
kill_all_anima
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product Anima"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/Anima.app" ]]; then
    APP_BUNDLE="/Applications/Anima.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/Anima.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/Anima.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/Anima.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set ANIMA_APP_BUNDLE to your installed Anima.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node anima.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node anima.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".anima", "anima.json"), "utf8");
