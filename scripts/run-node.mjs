...
const shouldBuild = (deps) => {
  if (deps.env.ANIMA_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.ANIMA_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[anima] ${message}\n`);
};

const runAnima = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["anima.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
