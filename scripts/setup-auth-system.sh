...
echo "  2. Subscribe to a topic (e.g., 'anima-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Anima message: Send warning via Anima itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/anima-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/anima-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/anima-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now anima-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status anima-auth-monitor.timer"
echo ""
echo "Quick commands:"
