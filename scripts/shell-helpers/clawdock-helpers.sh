#!/usr/bin/env bash
# ClawDock - Docker helpers for Anima
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    ...
    if [[ "$response" =~ ^[Nn] ]]; then
      ...
      echo "  export CLAWDOCK_DIR=/path/to/anima"
      return 1
    fi
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^ANIMA_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d anima-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart anima-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f anima-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec anima-gateway \
...
clawdock-cli() {
  _clawdock_compose run --rm anima-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build anima-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "ANIMA_GATEWAY_TOKEN=$token" anima-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" anima-gateway \
    bash -c './anima.mjs config set gateway.remote.token "$TOKEN" && ./anima.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec anima-gateway \
    bash -c "./anima.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart anima-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm anima-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec anima-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Anima${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (anima alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to anima project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.anima)${_CLR_RESET}"
