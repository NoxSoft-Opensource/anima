...
SERVER="${ANIMA_SERVER:-${ANIMA_SERVER:-l36}}"

# Check auth status
termux-toast "Checking Anima auth..."

STATUS=$(ssh "$SERVER" '$HOME/anima/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/anima/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|ANIMA_EXPIRING|ANIMA_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/anima/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|ANIMA_EXPIRED|ANIMA_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Anima won't work until re-auth"
                ;;
        esac
        ;;
