...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/anima/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Anima Re-Auth" -c "After getting key, run: ssh $SERVER '~/anima/scripts/mobile-reauth.sh'" --id anima-auth
        ;;
    *)
