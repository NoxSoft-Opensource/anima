 ...
 * `anima update`. Use it to verify the completion installation flow
...
\`anima update\`, \`anima doctor\`, and \`anima onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(theme.muted("  1. Remove the '# Anima Completion' block from your shell profile"));
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`anima completion --install\` later to enable.`));
    return;
  }
