---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Anima deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Anima cron scheduling for periodic checks, or version status checks on a machine running Anima (laptop, workstation, Pi, VPS).
---

# Anima Host Hardening

## Overview

Assess and harden the host running Anima, then align it to a user-defined risk tolerance without breaking access. Use Anima security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Anima changes the host firewall, SSH, or OS updates; it does not.
...
5. Anima gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Anima can access sensitive data.
...
- Credential hygiene notes (location of Anima creds, prefer disk encryption)
...
2. `npm view anima version` (published npm version)
...
Follow the durable-memory prompt format used by Anima compaction:
