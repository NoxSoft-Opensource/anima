...
export function listChannelSupportedActions(params: {
  ...
  const cfg = params.cfg ?? ({} as AnimaConfig);
...
export function listChannelAgentTools(params: { cfg?: AnimaConfig }): ChannelAgentTool[] {
...
function runPluginListActions(plugin: ChannelPlugin, cfg: AnimaConfig): ChannelMessageActionName[] {
  if (!plugin.actions?.listActions) {
    return [];
  }
  try {
