...
describe("Agent-specific tool filtering", () => {
  ...
  it("defaults apply_patch to workspace-only (blocks traversal)", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "anima-pi-tools-"));
...
*** End Patch`;
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createAnimaCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createAnimaCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createAnimaCodingTools({
      config: cfg,
    ...
    const defaultTools = createAnimaCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createAnimaCodingTools({
      config: cfg,
    ...
    const bobTools = createAnimaCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createAnimaCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createAnimaCodingTools({
      config: cfg,
