...
describe("createAnimaCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "anima-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    ...
    try {
      ...
      const contents = "Hello from anima read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "anima-sandbox");
    const sandbox = {
      enabled: true,
      sessionKey: "sandbox:test",
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "anima-workspace"),
    ...
    const tools = createAnimaCodingTools({ sandbox });
