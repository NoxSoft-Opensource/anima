...
import { __testing, createAnimaCodingTools } from "./pi-tools.js";
...
describe("createAnimaCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createAnimaCodingTools({ messageProvider: "discord" });
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "anima-depth-policy-"));
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "anima-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "anima-outside.txt");
