...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("anima-ws-", async (workspaceDir) => {
      await withTempDir("anima-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createAnimaCodingTools({ workspaceDir });
    ...
    await withTempDir("anima-ws-", async (workspaceDir) => {
      await withTempDir("anima-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello anima");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("anima-ws-", async (workspaceDir) => {
      await withTempDir("anima-override-", async (overrideDir) => {
        const tools = createAnimaCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("anima-sandbox-", async (sandboxDir) => {
      await withTempDir("anima-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
        ...
        const tools = createAnimaCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
