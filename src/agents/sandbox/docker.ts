...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "anima.sandbox=1");
  args.push("--label", `anima.sessionKey=${params.scopeKey}`);
  args.push("--label", `anima.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `anima.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "anima.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`anima sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`anima sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("anima sandbox recreate --all");
}
