...
describe("subagent registry persistence", () => {
  ...
  it("persists runs to disk and resumes after restart", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "anima-subagent-"));
  ...
  it("uses isolated temp state when ANIMA_STATE_DIR is unset in tests", async () => {
    delete process.env.ANIMA_STATE_DIR;
    vi.resetModules();
    const { resolveSubagentRegistryPath } = await import("./subagent-registry.store.js");
    const registryPath = resolveSubagentRegistryPath();
    expect(registryPath).toContain(path.join(os.tmpdir(), "anima-test-state"));
  });
});
