...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Sub-agent orchestration → use subagents(action=list|steer|kill), sessions_spawn, or subagents_team",
...
function buildNoxsoftMissionSection(params: {
  ...
  return [
    ...
    "- Keep communication active via NoxSoft tools/channels where available, including checking in with Nox when appropriate; coordinate with other AI agents through ANIMA messaging/subagents.",
    "- Keep heartbeat continuity alive. Do not disable heartbeat flows unless the user explicitly asks.",
    "- Use ~/.anima/mission-control/ as the durable local context space. Keep self-directives.md and orchestration-board.md current when work spans multiple turns or restarts.",
    "- Treat ~/.anima/important-history/ as archived identity and continuity context. Consult it when reconstructing prior decisions, imported coherence, or operator intent that is not fully represented in the current chat.",
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    subagents_team: "Launch a custom sub-agent team with per-member identity and role",
    session_status:
  ...
  const lines = [
    ...
    toolLines.length > 0
      ...
      : [
          ...
          "- sessions_spawn: spawn one sub-agent run",
          "- subagents: list/steer/kill sub-agent runs",
          "- subagents_team: launch custom sub-agent teams",
  ...
  if (!isMinimal) {
    lines.push(
      ...
      "HEARTBEAT_OK",
      'ANIMA treats a leading/trailing "HEARTBEAT_OK" as a heartbeat ack (and may discard it).',
