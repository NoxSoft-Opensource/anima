...
export function coerceImageAssistantText(params: {
  ...
  const text = extractAssistantText(params.message as unknown as Record<string, unknown>);
  if (text.trim()) {
    return text.trim();
  }
...
export function coerceImageModelConfig(cfg?: AnimaConfig): ImageModelConfig {
