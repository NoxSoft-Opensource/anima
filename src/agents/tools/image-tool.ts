...
function resolveDefaultModelRef(cfg?: AnimaConfig): {
  provider: string;
  model: string;
} {
  if (cfg) {
...
function pickMaxBytes(cfg?: AnimaConfig, maxBytesMb?: number): number | undefined {
...
async function runImagePrompt(params: {
  ...
  const effectiveCfg: AnimaConfig | undefined = params.cfg
    ? {
        ...params.cfg,
        agents: {
          ...params.cfg.agents,
  ...
  await ensureAnimaModelsJson(effectiveCfg, params.agentDir);
