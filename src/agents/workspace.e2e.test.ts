...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses ANIMA_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      ANIMA_HOME: "/srv/anima-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/anima-home"), ".anima", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".anima", "workspace-state.json"] as const;
