...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.ANIMA_PROFILE?.trim() || env.OPENCLAW_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".anima", `workspace-${profile}`);
  }
  return path.join(home, ".anima", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".anima";
