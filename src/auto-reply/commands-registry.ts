...
export function isCommandEnabled(cfg: AnimaConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: AnimaConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as AnimaConfig),
...
export function getCommandDetection(_cfg?: AnimaConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: AnimaConfig) {
