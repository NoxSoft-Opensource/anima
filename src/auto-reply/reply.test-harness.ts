...
type HomeEnvSnapshot = {
  ...
  ANIMA_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    ANIMA_AGENT_DIR: process.env.ANIMA_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".anima", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.ANIMA_STATE_DIR = path.join(home, ".anima");
    process.env.ANIMA_AGENT_DIR = path.join(home, ".anima", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".anima", "agent");
