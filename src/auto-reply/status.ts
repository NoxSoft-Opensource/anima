...
type AgentConfig = Partial<NonNullable<NonNullable<AnimaConfig["agents"]>["defaults"]>>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.anima/sessions/<SessionId>.jsonl)
  if (!sessionId) {
    return undefined;
  }
  let logPath: string;
...
export function buildHelpMessage(cfg?: AnimaConfig): string {
  const lines = ["ℹ️ Help", ""];
