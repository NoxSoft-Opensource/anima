import type { BrowserConfig, BrowserProfileConfig, AnimaConfig } from "../config/config.js";
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "anima" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_ANIMA_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "anima" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_ANIMA_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_ANIMA_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an ANIMA browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for anima/anima CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: AnimaConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_ANIMA_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "anima";
