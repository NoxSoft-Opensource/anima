...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredAnimaTmpDir: vi.fn(() => "/tmp/anima"),
}));
vi.mock("../infra/tmp-anima-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredAnimaTmpDir.mockReturnValue("/tmp/anima");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    ...
    tmpDirMocks.resolvePreferredAnimaTmpDir.mockReturnValue("/tmp/anima-preferred");
    setPwToolsCoreCurrentPage({ on, off });
    ...
    const expectedRootedDownloadsDir = path.join(path.sep, "tmp", "anima-preferred", "downloads");
    const expectedDownloadsTail = `${path.join("tmp", "anima-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredAnimaTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "anima-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "anima-preferred", "downloads")}${path.sep}`),
    );
  });
