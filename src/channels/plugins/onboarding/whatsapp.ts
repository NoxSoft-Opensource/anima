...
function setWhatsAppDmPolicy(cfg: AnimaConfig, dmPolicy: DmPolicy): AnimaConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: AnimaConfig, allowFrom?: string[]): AnimaConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: AnimaConfig, selfChatMode: boolean): AnimaConfig {
  return mergeWhatsAppConfig(cfg, { selfChatMode });
}

async function detectWhatsAppLinked(cfg: AnimaConfig, accountId: string): Promise<boolean> {
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("anima channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
