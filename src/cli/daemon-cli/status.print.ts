...
export function printDaemonStatus(status: DaemonStatus, opts: { json: boolean }) {
  ...
  if (service.runtime?.cachedLabel) {
    ...
    const labelValue = resolveGatewayLaunchAgentLabel(env.ANIMA_PROFILE);
    defaultRuntime.error(
      errorText(
    ...
    defaultRuntime.error(
      errorText(`Then reinstall: ${formatCliCommand("anima gateway install")}`),
    );
    spacer();
  }
  ...
  defaultRuntime.log(`${label("Troubleshooting:")} https://docs.anima.ai/troubleshooting`);
}
