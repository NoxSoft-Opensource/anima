...
export function registerStatusHealthSessionsCommands(program: Command) {
  program
    .command("status")
    .description("Channel health and session activity overview")
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ...
          ["anima status --deep", "Run channel probes (WA + Telegram + Discord + Slack + Signal)."],
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.noxsoft.net/anima/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    .command("health")
    .description("Probe the running Gateway for live health")
    .option("--json", "Output JSON instead of text", false)
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.noxsoft.net/anima/cli/health")}\n`,
    )
    .action(async (opts) => {
      const verbose = resolveVerbose(opts);
      setVerbose(verbose);
  ...
  program
    .command("sessions")
    .description("List and inspect conversation sessions")
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.noxsoft.net/anima/cli/sessions")}\n`,
    )
    .action(async (opts) => {
