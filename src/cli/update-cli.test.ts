...
vi.mock("../infra/anima-root.js", () => ({
  resolveAnimaPackageRoot: vi.fn(),
}));
...
const { resolveAnimaPackageRoot } = await import("../infra/anima-root.js");
...
describe("update-cli", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "anima-update-tests-"));
  });
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = await createCaseDir("anima-update");
    setTty(false);
    readPackageVersion.mockResolvedValue("2.0.0");

    vi.mocked(resolveAnimaPackageRoot).mockResolvedValue(tempDir);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveAnimaPackageRoot).mockReset();
    ...
    vi.mocked(resolveAnimaPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("@noxsoft/anima");
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await createCaseDir("anima-update-wizard");
    const previousGitDir = process.env.ANIMA_GIT_DIR;
    try {
      setTty(true);
      process.env.ANIMA_GIT_DIR = tempDir;
      ...
      process.env.ANIMA_GIT_DIR = previousGitDir;
    }
  });
});
