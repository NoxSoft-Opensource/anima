...
const UPDATE_QUIPS = [
  ...
  "Anima is updated and battle-ready.",
  "Update done! Check the changelog or just trust me, it's good.",
  "Rebuilt from npm bytes. Stronger now.",
  ...
  "Upgrade complete. Stability phase engaged.",
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("anima completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
async function maybeRestartService(params: {
  ...
  if (params.shouldRestart) {
    ...
    try {
      ...
      if (!params.opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.ANIMA_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.ANIMA_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!params.opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("anima gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
  ...
  if (!params.opts.json) {
    ...
    if (params.result.mode === "npm" || params.result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("anima doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("anima gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("anima gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      ...
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g anima@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g anima@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
