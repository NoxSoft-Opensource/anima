...
export function listAgentEntries(cfg: AnimaConfig): AgentEntry[] {
...
function resolveAgentName(cfg: AnimaConfig, agentId: string) {
...
function resolveAgentModel(cfg: AnimaConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: AnimaConfig): AgentSummary[] {
