...
async function runGatewayHealthCheck(params: {
  ...
  const token = params.cfg.gateway?.auth?.token ?? process.env.ANIMA_GATEWAY_TOKEN;
  const password = params.cfg.gateway?.auth?.password ?? process.env.ANIMA_GATEWAY_PASSWORD;
...
export async function runConfigureWizard(
  ...
  try {
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("anima doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
