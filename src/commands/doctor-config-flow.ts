...
function stripUnknownConfigKeys(config: AnimaConfig): {
  config: AnimaConfig;
  removed: string[];
} {
...
function noteOpencodeProviderOverrides(cfg: AnimaConfig) {
...
function scanTelegramAllowFromUsernameEntries(cfg: AnimaConfig): TelegramAllowFromUsernameHit[] {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: AnimaConfig): Promise<{
  config: AnimaConfig;
  changes: string[];
}> {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("anima doctor --fix")}" to apply these changes.`);
    }
  }
