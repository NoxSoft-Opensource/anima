...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "ANIMA_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("ANIMA_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv ANIMA_GATEWAY_TOKEN");
    expect(message).not.toContain("ANIMA_GATEWAY_PASSWORD");
  });
