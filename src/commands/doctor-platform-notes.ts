...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".anima", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: AnimaConfig): boolean {
  const localToken =
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const deprecatedLaunchctlEntries = [
    ["ANIMA_GATEWAY_TOKEN", await getenv("ANIMA_GATEWAY_TOKEN")],
    ["ANIMA_GATEWAY_PASSWORD", await getenv("ANIMA_GATEWAY_PASSWORD")],
  ...
  if (deprecatedLaunchctlEntries.length > 0) {
    const lines = [
      ...
      ...deprecatedLaunchctlEntries.map(
        ([key]) => `- \`${key}\` is set; use \`ANIMA_${key.slice(key.indexOf("_") + 1)}\` instead.`,
      ),
    ];
    (deps?.noteFn ?? note)(lines.join("\n"), "Gateway (macOS)");
  }

  const tokenEntries = [["ANIMA_GATEWAY_TOKEN", await getenv("ANIMA_GATEWAY_TOKEN")]] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "ANIMA_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteDeprecatedLegacyEnvVars(
  ...
  const entries = Object.entries(env)
    .filter(([key, value]) => key.startsWith("ANIMA_") && value?.trim())
    .map(([key]) => key);
  if (entries.length === 0) {
    return;
  }
