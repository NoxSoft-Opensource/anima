...
import type { AnimaConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: AnimaConfig) {
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("anima configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("anima config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("anima doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("anima config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('anima config set session.dmScope "per-channel-peer"') +
