...
function resolveMode(cfg: AnimaConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  let cfg: AnimaConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    ...
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("anima setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("anima doctor --fix")}" to apply changes.`);
  }
