...
describe("doctor command", () => {
  ...
  it("does not warn when only the active workspace is present", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      ...
      valid: true,
      config: {
        agents: { defaults: { workspace: "/Users/steipete/anima" } },
      },
      issues: [],
      legacyIssues: [],
    });
    ...
    const legacyPath = path.join("/Users/steipete", "anima");
    const legacyAgentsPath = path.join(legacyPath, "AGENTS.md");
    const existsSpy = vi.spyOn(fs, "existsSync").mockImplementation((value) => {
      if (value === "/Users/steipete/anima" || value === legacyPath || value === legacyAgentsPath) {
        return true;
      }
