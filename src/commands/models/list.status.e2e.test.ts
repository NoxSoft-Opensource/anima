...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveAnimaAgentDir: vi.fn().mockReturnValue("/tmp/anima-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/anima-agent"),
    ...
    resolveAuthStorePathForDisplay: vi.fn().mockReturnValue("/tmp/anima-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveAnimaAgentDir: mocks.resolveAnimaAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveAnimaAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/anima-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/anima-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/anima-agent-custom");
