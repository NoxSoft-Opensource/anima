...
export async function updateConfig(
  mutator: (cfg: AnimaConfig) => AnimaConfig,
): Promise<AnimaConfig> {
...
export function resolveModelTarget(params: { raw: string; cfg: AnimaConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: AnimaConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("anima agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
