...
const requireAgentDir = () => {
  const agentDir = process.env.ANIMA_AGENT_DIR;
  if (!agentDir) {
    throw new Error("ANIMA_AGENT_DIR not set");
  }
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.ANIMA_STATE_DIR;
  const previousAgentDir = process.env.ANIMA_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      delete process.env.ANIMA_AGENT_DIR;
    } else {
      process.env.ANIMA_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.ANIMA_OAUTH_DIR;
  });

  it("writes auth-profiles.json under ANIMA_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "anima-oauth-"));
    process.env.ANIMA_STATE_DIR = tempStateDir;
    process.env.ANIMA_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.ANIMA_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to ANIMA_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "anima-minimax-"));
    process.env.ANIMA_STATE_DIR = tempStateDir;
    process.env.ANIMA_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.ANIMA_AGENT_DIR;
