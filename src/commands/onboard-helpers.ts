...
export function summarizeExistingConfig(config: AnimaConfig): string {
  const rows: string[] = [];
...
export async function ensureWorkspaceAndSessions(
  workspaceDir: string,
  runtime: RuntimeEnv,
  options?: { skipBootstrap?: boolean; agentId?: string; seedBootstrapOnFirstRun?: boolean },
) {
  const ws = await ensureAgentWorkspace({
    dir: workspaceDir,
    ensureBootstrapFiles: !options?.skipBootstrap,
    seedBootstrapOnFirstRun: options?.seedBootstrapOnFirstRun,
  });
