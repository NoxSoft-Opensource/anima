...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "anima-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.ANIMA_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "anima");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.ANIMA_STATE_DIR = stateDir;
    process.env.ANIMA_CONFIG_PATH = path.join(stateDir, "anima.json");
