...
beforeAll(() => {
  previousProfile = process.env.ANIMA_PROFILE;
  process.env.ANIMA_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.ANIMA_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/anima-root.js", () => ({
  resolveAnimaPackageRoot: vi.fn().mockResolvedValue("/tmp/anima"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/anima/pnpm-lock.yaml",
      markerPath: "/tmp/anima/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = (runtime.log as vi.Mock).mock.calls.map((c) => String(c[0]));
    expect(logs.some((l) => l.includes("Anima status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("anima status --all") ||
          l.includes("anima --profile isolated status --all") ||
          l.includes("anima status --all") ||
          l.includes("anima --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.ANIMA_GATEWAY_TOKEN = "abcd1234";
    try {
