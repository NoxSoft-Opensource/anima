...
const ETC_ANIMA_DIR = path.join(ROOT_DIR, "etc", "anima");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "anima.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcAnimaPath(...parts: string[]) {
  return path.join(ETC_ANIMA_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcAnimaPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "anima.json"))).toEqual({
      shared: true,
    });
  });
});
