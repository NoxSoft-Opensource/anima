...
import type { AnimaConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
...
import { compareAnimaVersions } from "./version.js";
...
function coerceConfig(value: unknown): AnimaConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as AnimaConfig;
}
...
function stampConfigVersion(cfg: AnimaConfig): AnimaConfig {
...
function warnIfConfigFromFuture(cfg: AnimaConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareAnimaVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as AnimaConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): AnimaConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as AnimaConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: AnimaConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfig =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as AnimaConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.ANIMA_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.ANIMA_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.ANIMA_WATCH_SESSION === "string" &&
        deps.env.ANIMA_WATCH_SESSION.trim().length > 0
          ? deps.env.ANIMA_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.ANIMA_WATCH_COMMAND === "string" &&
        deps.env.ANIMA_WATCH_COMMAND.trim().length > 0
          ? deps.env.ANIMA_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `ANIMA_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.ANIMA_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.ANIMA_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
