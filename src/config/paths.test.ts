...
describe("state + config path candidates", () => {
  ...
  it("prefers ~/.anima when it exists and legacy dir is missing", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "anima-state-"));
    try {
  ...
  it("CONFIG_PATH prefers existing config when present", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "anima-config-"));
    try {
  ...
  it("respects state dir overrides when config is missing", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "anima-config-override-"));
    try {
      const legacyDir = path.join(root, ".anima");
      await fs.mkdir(legacyDir, { recursive: true });
