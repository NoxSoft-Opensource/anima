...
function resolveChannelConfig(cfg: AnimaConfig, channelId: string): Record<string, unknown> | null {
  const channels = cfg.channels as Record<string, unknown> | undefined;
  const entry = channels?.[channelId];
  return isRecord(entry) ? entry : null;
}

function isTelegramConfigured(cfg: AnimaConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: AnimaConfig, env: NodeJS.ProcessEnv): boolean {
...
function isIrcConfigured(cfg: AnimaConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: AnimaConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: AnimaConfig): boolean {
...
function isIMessageConfigured(cfg: AnimaConfig): boolean {
...
function isWhatsAppConfigured(cfg: AnimaConfig): boolean {
...
function isGenericChannelConfigured(cfg: AnimaConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: AnimaConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: AnimaConfig, providerId: string): boolean {
...
function resolveConfiguredPlugins(cfg: AnimaConfig, env: NodeJS.ProcessEnv): PluginEnableChange[] {
...
function isPluginExplicitlyDisabled(cfg: AnimaConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: AnimaConfig, pluginId: string): boolean {
...
function ensureAllowlisted(cfg: AnimaConfig, pluginId: string): AnimaConfig {
...
function registerPluginEntry(cfg: AnimaConfig, pluginId: string): AnimaConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
