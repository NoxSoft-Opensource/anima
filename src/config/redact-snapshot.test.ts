...
import type { ConfigFileSnapshot } from "./types.anima.js";
import {
  REDACTED_SENTINEL,
...
function makeSnapshot(config: Record<string, unknown>, raw?: string): ConfigFileSnapshot {
  return {
    path: "/home/user/.anima/config.json5",
    exists: true,
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = AnimaSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "AnimaConfig";
    const hints = mapSensitivePaths(AnimaSchema, "", {});
