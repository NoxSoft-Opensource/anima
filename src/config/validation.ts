...
import type { AnimaConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: AnimaConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: AnimaConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = AnimaSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as AnimaConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as AnimaConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as AnimaConfig,
  };
}
