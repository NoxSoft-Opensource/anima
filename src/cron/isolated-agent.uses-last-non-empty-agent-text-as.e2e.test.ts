...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "anima-cron-" });
}
...
async function writeSessionStore(
  ...
  const dir = path.join(home, ".anima", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<AnimaConfig> = {},
): AnimaConfig {
  const base: AnimaConfig = {
    agents: {
      defaults: {
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".anima", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
