...
async function writeLaunchctlStub(binDir: string) {
  if (process.platform === "win32") {
    ...
    await fs.writeFile(
      ...
      [
        ...
        "const logPath = process.env.ANIMA_TEST_LAUNCHCTL_LOG;",
        "if (logPath) {",
  ...
  await fs.writeFile(
    ...
    [
      "#!/bin/sh",
      'log_path="${ANIMA_TEST_LAUNCHCTL_LOG:-}"',
      'if [ -n "$log_path" ]; then',
      '  line=""',
      '  for arg in "$@"; do',
      '    if [ -n "$line" ]; then',
...
async function withLaunchctlStub(
  ...
  const originalLogPath = process.env.ANIMA_TEST_LAUNCHCTL_LOG;
  const originalListOutput = process.env.ANIMA_TEST_LAUNCHCTL_LIST_OUTPUT;

  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "anima-launchctl-test-"));
  try {
    ...
    process.env.ANIMA_TEST_LAUNCHCTL_LOG = logPath;
    process.env.ANIMA_TEST_LAUNCHCTL_LIST_OUTPUT = options.listOutput ?? "";
    process.env.PATH = `${binDir}${path.delimiter}${originalPath ?? ""}`;

    await run({
      env: {
        HOME: homeDir,
        ANIMA_PROFILE: "default",
      },
      logPath,
    });
  } finally {
    ...
    if (originalLogPath === undefined) {
      delete process.env.ANIMA_TEST_LAUNCHCTL_LOG;
    } else {
      process.env.ANIMA_TEST_LAUNCHCTL_LOG = originalLogPath;
    }
    if (originalListOutput === undefined) {
      delete process.env.ANIMA_TEST_LAUNCHCTL_LIST_OUTPUT;
    } else {
      process.env.ANIMA_TEST_LAUNCHCTL_LIST_OUTPUT = originalListOutput;
    }
...
describe("resolveLaunchAgentPlistPath", () => {
  it("uses default label when ANIMA_PROFILE is default", () => {
    const env = { HOME: "/Users/test", ANIMA_PROFILE: "default" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/net.noxsoft.anima.gateway.plist",
    );
  });

  it("uses default label when ANIMA_PROFILE is unset", () => {
  ...
  it("uses profile-specific label when ANIMA_PROFILE is set to a custom value", () => {
    const env = { HOME: "/Users/test", ANIMA_PROFILE: "jbphoenix" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/net.noxsoft.anima.jbphoenix.plist",
    );
  });

  it("prefers ANIMA_LAUNCHD_LABEL over ANIMA_PROFILE", () => {
    const env = {
      HOME: "/Users/test",
      ANIMA_PROFILE: "jbphoenix",
      ANIMA_LAUNCHD_LABEL: "com.custom.label",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
  ...
  it("trims whitespace from ANIMA_LAUNCHD_LABEL", () => {
    const env = {
      HOME: "/Users/test",
      ANIMA_LAUNCHD_LABEL: "  com.custom.label  ",
    };
  ...
  it("ignores empty ANIMA_LAUNCHD_LABEL and falls back to profile", () => {
    const env = {
      HOME: "/Users/test",
      ANIMA_PROFILE: "myprofile",
      ANIMA_LAUNCHD_LABEL: "   ",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/net.noxsoft.anima.myprofile.plist",
    );
  });
  ...
  it("trims whitespace from ANIMA_PROFILE", () => {
    const env = { HOME: "/Users/test", ANIMA_PROFILE: "  myprofile  " };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/net.noxsoft.anima.myprofile.plist",
    );
  });
