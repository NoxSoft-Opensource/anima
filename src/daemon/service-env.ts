...
export function buildServiceEnvironment(params: {
  ...
  const stateDir = env.ANIMA_STATE_DIR;
  const configPath = env.ANIMA_CONFIG_PATH;
  return {
    HOME: env.HOME,
    PATH: buildMinimalServicePath({ env }),
    ANIMA_PROFILE: profile,
    ANIMA_STATE_DIR: stateDir,
    ANIMA_CONFIG_PATH: configPath,
    ANIMA_GATEWAY_PORT: String(port),
    ANIMA_GATEWAY_TOKEN: token,
    ANIMA_LAUNCHD_LABEL: resolvedLaunchdLabel,
    ANIMA_SYSTEMD_UNIT: systemdUnit,
    ANIMA_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    ANIMA_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  return {
    ...
    ANIMA_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    ANIMA_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    ANIMA_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    ANIMA_LOG_PREFIX: "node",
    ANIMA_SERVICE_MARKER: NODE_SERVICE_MARKER,
    ANIMA_SERVICE_KIND: NODE_SERVICE_KIND,
    ANIMA_SERVICE_VERSION: VERSION,
  };
}
