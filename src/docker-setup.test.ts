...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "anima-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    ANIMA_GATEWAY_TOKEN: "test-token",
    ANIMA_CONFIG_DIR: join(sandbox.rootDir, "config"),
    ANIMA_WORKSPACE_DIR: join(sandbox.rootDir, "anima"),
  };
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = spawnSync("bash", [sandbox.scriptPath], {
      ...
      env: createEnv(sandbox, {
        ANIMA_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
        ANIMA_EXTRA_MOUNTS: undefined,
        ANIMA_HOME_VOLUME: "anima-home",
      }),
      stdio: ["ignore", "ignore", "pipe"],
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(sandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("ANIMA_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("ANIMA_EXTRA_MOUNTS=");
    expect(envFile).toContain("ANIMA_HOME_VOLUME=anima-home");
    const extraCompose = await readFile(join(sandbox.rootDir, "docker-compose.extra.yml"), "utf8");
    expect(extraCompose).toContain("anima-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("anima-home:");
    const log = await readFile(sandbox.logPath, "utf8");
    expect(log).toContain("--build-arg ANIMA_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
