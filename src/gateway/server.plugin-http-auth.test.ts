...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.ANIMA_CONFIG_PATH;
  const prevDisableCache = process.env.ANIMA_DISABLE_CONFIG_CACHE;

  const dir = await mkdtemp(path.join(os.tmpdir(), "anima-plugin-http-auth-test-"));
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.ANIMA_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.ANIMA_DISABLE_CONFIG_CACHE;
    } else {
      process.env.ANIMA_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
