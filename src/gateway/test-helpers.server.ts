...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.ANIMA_STATE_DIR;
  previousConfigPath = process.env.ANIMA_CONFIG_PATH;
  previousSkipBrowserControl = process.env.ANIMA_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.ANIMA_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.ANIMA_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.ANIMA_BUNDLED_PLUGINS_DIR;
  previousSkipChannels = process.env.ANIMA_SKIP_CHANNELS;
  previousSkipProviders = process.env.ANIMA_SKIP_PROVIDERS;
  previousSkipCron = process.env.ANIMA_SKIP_CRON;
  previousMinimalGateway = process.env.ANIMA_TEST_MINIMAL_GATEWAY;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "anima-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.ANIMA_STATE_DIR = path.join(tempHome, ".anima");
  delete process.env.ANIMA_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.ANIMA_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.ANIMA_SKIP_GMAIL_WATCHER = "1";
  process.env.ANIMA_SKIP_CANVAS_HOST = "1";
  process.env.ANIMA_SKIP_CHANNELS = "1";
  process.env.ANIMA_SKIP_PROVIDERS = "1";
  process.env.ANIMA_SKIP_CRON = "1";
  process.env.ANIMA_TEST_MINIMAL_GATEWAY = "1";
  process.env.ANIMA_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "anima-test-no-bundled-extensions")
    : "anima-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "anima-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".anima-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.ANIMA_STATE_DIR = previousStateDir;
    }
    if (previousConfigPath === undefined) {
      delete process.env.ANIMA_CONFIG_PATH;
    } else {
      process.env.ANIMA_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.ANIMA_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.ANIMA_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.ANIMA_SKIP_GMAIL_WATCHER;
    } else {
      process.env.ANIMA_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.ANIMA_SKIP_CANVAS_HOST;
    } else {
      process.env.ANIMA_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
    if (previousBundledPluginsDir === undefined) {
      delete process.env.ANIMA_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.ANIMA_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
    }
    if (previousSkipChannels === undefined) {
      delete process.env.ANIMA_SKIP_CHANNELS;
    } else {
      process.env.ANIMA_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.ANIMA_SKIP_PROVIDERS;
    } else {
      process.env.ANIMA_SKIP_PROVIDERS = previousSkipProviders;
    }
    if (previousSkipCron === undefined) {
      delete process.env.ANIMA_SKIP_CRON;
    } else {
      process.env.ANIMA_SKIP_CRON = previousSkipCron;
    }
    if (previousMinimalGateway === undefined) {
      delete process.env.ANIMA_TEST_MINIMAL_GATEWAY;
    } else {
      process.env.ANIMA_TEST_MINIMAL_GATEWAY = previousMinimalGateway;
    }
  }
...
export async function startServerWithClient(
  ...
  const prev = process.env.ANIMA_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.ANIMA_GATEWAY_TOKEN = fallbackToken;
  }
