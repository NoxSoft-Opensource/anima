...
2. **Appends to log file** - Writes a JSON line to `~/.anima/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.anima/logs/commands.log ~/.anima/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/anima`:
   ```
   /home/username/.anima/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
