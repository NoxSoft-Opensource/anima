...
`;
    ...
    expect(parsed.anima.emoji).toBe("💾");
    expect(parsed.anima.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.anima.emoji).toBe("📝");
    expect(parsed.anima.events).toEqual(["command"]);
    expect(parsed.anima.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.anima.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"anima": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"anima": {"events": ["test"]}}');
  });
...
describe("resolveAnimaMetadata", () => {
  it("extracts anima metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveAnimaMetadata(frontmatter);
  ...
  it("returns undefined when anima key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    ...
    expect(result?.install?.[1].package).toBe("@anima/hook");
  });
...
`;
    ...
    const anima = resolveAnimaMetadata(frontmatter);
    expect(anima).toBeDefined();
    expect(anima?.emoji).toBe("💾");
    expect(anima?.events).toEqual(["command:new"]);
    expect(anima?.requires?.config).toEqual(["workspace.dir"]);
    expect(anima?.install?.[0].kind).toBe("bundled");
  });
