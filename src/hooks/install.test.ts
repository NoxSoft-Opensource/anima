...
const fixtureRoot = path.join(os.tmpdir(), `anima-hook-install-${randomUUID()}`);
let tempDirIndex = 0;
...
beforeAll(async () => {
  ...
  zipHooks.file(
    ...
    JSON.stringify({
      name: "@anima/zip-hooks",
      version: "0.0.1",
      anima: { hooks: ["./hooks/zip-hook"] },
    }),
  );
  zipHooks.file(
    ...
    [
      ...
      'metadata: {"anima":{"events":["command:new"]}}',
      "---",
      "",
      "# Zip Hook",
    ].join("\n"),
  ...
  const makeTarWithPackage = async (params: {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "0.0.1",
        anima: { hooks: [`./hooks/${params.hookName}`] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  const makeNpmPackTgz = async (): Promise<Buffer> => {
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({
        name: "@anima/test-hooks",
        version: "0.0.1",
        anima: { hooks: ["./hooks/one-hook"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
...
describe("installHooksFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(argv).toEqual(["npm", "pack", "@anima/test-hooks@0.0.1", "--ignore-scripts"]);
