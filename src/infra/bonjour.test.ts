...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    process.env.ANIMA_MDNS_HOSTNAME = "test-host";
    process.env.ANIMA_MDNS_HOSTNAME = "test-host";
    ...
    expect(gatewayCall?.[0]?.type).toBe("anima-gw");
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("anima (ANIMA)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("anima");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("anima.local");
