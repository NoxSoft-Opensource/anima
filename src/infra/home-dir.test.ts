...
describe("resolveEffectiveHomeDir", () => {
  it("prefers ANIMA_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(path.resolve("/srv/anima-home"));
  });
  ...
  it("expands ANIMA_HOME when set to ~", () => {
    const env = {
      ANIMA_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for ANIMA_HOME", () => {
    const result = resolveRequiredHomeDir(
      { ANIMA_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when ANIMA_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ ANIMA_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { ANIMA_HOME: "/srv/anima-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/anima-home")}/x`);
  });
