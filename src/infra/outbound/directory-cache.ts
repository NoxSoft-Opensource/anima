...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: AnimaConfig | null = null;
  ...
  get(key: string, cfg: AnimaConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: AnimaConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: AnimaConfig): void {
