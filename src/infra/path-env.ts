...
function candidateBinDirs(opts: EnsureAnimaPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `anima` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "anima");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.ANIMA_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "anima"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `anima` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureAnimaCliOnPath(opts: EnsureAnimaPathOpts = {}) {
  if (isTruthyEnvValue(process.env.ANIMA_PATH_BOOTSTRAPPED)) {
    return;
  }
