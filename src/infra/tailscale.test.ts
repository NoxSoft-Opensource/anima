...
describe("tailscale helpers", () => {
  const originalForcedBinary = process.env.ANIMA_TEST_TAILSCALE_BINARY;

  beforeEach(() => {
    process.env.ANIMA_TEST_TAILSCALE_BINARY = "tailscale";
  });

  afterEach(() => {
    if (originalForcedBinary === undefined) {
      delete process.env.ANIMA_TEST_TAILSCALE_BINARY;
    } else {
      process.env.ANIMA_TEST_TAILSCALE_BINARY = originalForcedBinary;
    }
    vi.restoreAllMocks();
  });
