...
export const POSIX_ANIMA_TMP_DIR = "/tmp/anima";

type ResolvePreferredAnimaTmpDirOptions = {
...
export function resolvePreferredAnimaTmpDir(
  options: ResolvePreferredAnimaTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "anima" : `anima-${uid}`;
  ...
  try {
    ...
    mkdirSync(POSIX_ANIMA_TMP_DIR, { recursive: true, mode: 0o700 });
    try {
      const preferred = lstatSync(POSIX_ANIMA_TMP_DIR);
      if (!preferred.isDirectory() || preferred.isSymbolicLink()) {
        return fallback();
