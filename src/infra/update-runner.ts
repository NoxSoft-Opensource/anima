...
const DEFAULT_PACKAGE_NAME = "anima";
...
function normalizeTag(tag?: string) {
  ...
  if (trimmed.startsWith("anima@")) {
    return trimmed.slice("anima@".length);
  }
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "anima-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "anima.mjs");
    ...
    const doctorStep = await runStep(
      step("anima doctor", doctorArgv, gitRoot, { ANIMA_UPDATE_IN_PROGRESS: "1" }),
    );
