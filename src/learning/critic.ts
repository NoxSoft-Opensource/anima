...
const SHADOW_DETECTORS: ShadowDetector[] = [
  ...
  },
  {
    ...
    detect(output) {
      ...
      const readMatches = output.match(/(?:reading|read|opened|loaded)\s+(?:file|[\w/.-]+\.\w+)/gi);
...
export class SessionCritic {
  ...
  async evaluate(
    ...
    const taskSuccess = session.status === "completed" && session.exitCode === 0;

    const efficiencyScore = this.calculateEfficiency(session.durationMs, costUsd, budgetUsd);
    ...
    const shadowPenalty =
      shadowPatterns.length > 0
        ? shadowPatterns.reduce((sum, sp) => sum + sp.confidence, 0) / shadowPatterns.length
        : 0;
    ...
    const notes = this.generateNotes(taskSuccess, efficiencyScore, shadowPatterns, session);
   ...
   */
  private detectShadowPatterns(output: string, context: DetectionContext): ShadowDetection[] {
   ...
   */
  private calculateEfficiency(durationMs: number, costUsd: number, budgetUsd: number): number {
  ...
  private generateNotes(
    ...
    if (taskSuccess) {
      parts.push(`Task completed successfully (exit code ${session.exitCode}).`);
    } else {
      parts.push(`Task ${session.status} with exit code ${session.exitCode}.`);
    }
