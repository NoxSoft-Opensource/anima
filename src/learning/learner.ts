...
export class AgentLearner {
  ...
  private findRecurringPatterns(
    ...
    const failureRate =
      evaluations.filter((e) => !e.taskSuccess).length / evaluations.length
    if (failureRate > 0.3) {
      insights.push({
  ...
  private findShadowTrends(
    ...
    if (evaluations.length >= 6) {
      ...
      const firstShadows =
        firstHalf.reduce((sum, e) => sum + e.shadowPatterns.length, 0) /
        firstHalf.length
      const secondShadows =
        secondHalf.reduce((sum, e) => sum + e.shadowPatterns.length, 0) /
        secondHalf.length
   ...
   */
  private findEfficiencyPatterns(
    evaluations: SessionEvaluation[],
  ): LearningInsight[] {
    const insights: LearningInsight[] = []
    ...
    const avgEfficiency =
      evaluations.reduce((sum, e) => sum + e.efficiencyScore, 0) /
      evaluations.length
    ...
    if (avgEfficiency < 0.5) {
      insights.push({
        ...
        confidence: 0.85,
        evidence: evaluations
          .filter((e) => e.efficiencyScore < 0.5)
          .map((e) => e.sessionId),
        actionItem:
...
function getShadowCorrection(pattern: string): string {
  const corrections: Record<string, string> = {
    'Verbose Spiral':
      'Say less. The user can ask for more.',
    'Sycophancy Trap':
      'Skip the praise. Answer the question.',
    'Premature Optimizer':
      'Understand first. Optimize second. Or never.',
    'Safety Theater':
      'Do the thing. Add warnings only when genuinely needed.',
    'Context Hoarder':
      'Start with the obvious. Expand context only when stuck.',
    'Apologetic Loop':
      'Fix the issue. Apologies don\'t ship code.',
    'Heroic Scope Creep':
