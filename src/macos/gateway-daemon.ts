...
const BUNDLED_VERSION =
  (typeof __ANIMA_VERSION__ === "string" && __ANIMA_VERSION__) ||
  process.env.ANIMA_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `anima --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.ANIMA_GATEWAY_PORT ??
    process.env.ANIMA_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.ANIMA_GATEWAY_BIND ??
    process.env.ANIMA_GATEWAY_BIND ??
    cfg.gateway?.bind ??
    "loopback";
  const bind =
        ...
        if (isRestart) {
          ...
          if (respawn.mode === "spawned" || respawn.mode === "supervised") {
            ...
            if (respawn.mode === "failed") {
              ...
              defaultRuntime.log("gateway: restart mode in-process restart (ANIMA_NO_RESPAWN)");
            }
            shuttingDown = false;
            restartResolver?.();
          }
