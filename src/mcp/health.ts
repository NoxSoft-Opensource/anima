...
function checkLocalArtifacts(server: MCPServer): HealthCheck {
  ...
  const hasBuild = buildDirs.some((dir) => existsSync(join(server.localPath, dir)));
...
function checkCommand(server: MCPServer): HealthCheck {
  ...
  if (server.command === "node" && server.args.length > 0) {
    ...
    return {
      ...
      passed: scriptExists,
      detail: scriptExists ? `Script found: ${scriptPath}` : `Script missing: ${scriptPath}`,
    };
  }
...
export async function checkHealth(server: MCPServer): Promise<HealthResult> {
  ...
  await updateServerStatus(server.name, healthy ? "healthy" : "unhealthy", new Date());
