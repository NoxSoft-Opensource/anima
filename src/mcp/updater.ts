...
export async function updateServer(server: MCPServer): Promise<UpdateResult> {
  ...
  // Step 3: pnpm install
  const install = await runStep("pnpm-install", "pnpm", ["install", "--frozen-lockfile"], cwd);
  steps.push(install);

  if (!install.success) {
    // Try without frozen lockfile
    const installRetry = await runStep("pnpm-install-retry", "pnpm", ["install"], cwd);
  ...
  if (!build.success) {
    // Rollback
    const reset = await runStep("git-reset-rollback", "git", ["reset", "--hard", "HEAD~1"], cwd);
    steps.push(reset);

    const stashPop = await runStep("git-stash-pop", "git", ["stash", "pop"], cwd);
    steps.push(stashPop);
