...
import { loadAnimaPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `anima-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.ANIMA_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "anima.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    ...
    process.env.ANIMA_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadAnimaPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.ANIMA_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadAnimaPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadAnimaPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
