...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: AnimaPluginDefinition;
  register?: AnimaPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as AnimaPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as AnimaPluginDefinition;
...
export function loadAnimaPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverAnimaPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias || pluginSdkAccountIdAlias
      ? {
          alias: {
            ...(pluginSdkAlias ? { "anima/plugin-sdk": pluginSdkAlias } : {}),
            ...(pluginSdkAccountIdAlias
              ? { "anima/plugin-sdk/account-id": pluginSdkAccountIdAlias }
              : {}),
          },
        }
      : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: AnimaPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as AnimaPluginModule;
    } catch (err) {
