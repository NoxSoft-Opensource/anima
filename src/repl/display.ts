...
export function statusPanel(stats: {
  ...
  const hbStatus = stats.heartbeatRunning
    ? `${s}running${r}`
    : `${e}stopped${r}`

  const lastBeat = stats.lastBeat
    ? stats.lastBeat.toLocaleTimeString()
    : 'never'

  const nextBeat = stats.nextBeat
    ? stats.nextBeat.toLocaleTimeString()
    : 'n/a'
