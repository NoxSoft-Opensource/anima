 ...
 * Non-command input (not starting with :) is queued as a task.
...
export class AnimaRepl {
  ...
  async start(): Promise<void> {
    ...
    const identityName = identity.soul.split('\n').find((l) =>
      l.startsWith('#'),
    )?.replace(/^#+\s*/, '') || 'ANIMA'
    ...
    process.stdout.write(
      `${colors.muted}  Type anything to queue a task, or :help for commands.${colors.reset}\n\n`,
    )
  ...
  private async handleInput(line: string): Promise<void> {
    ...
    if (line.startsWith(':')) {
      ...
      if (!command) {
        process.stdout.write(
          formatError(`Unknown command: :${cmdName}. Try :help`) + '\n',
        )
        this.rl?.prompt()
        return
      }
    ...
    // Non-command input — queue as task
    const item = this.ctx.queue.enqueue(line, 'normal', 'repl')
    process.stdout.write(
      formatSuccess(`Queued task ${item.id} (${item.priority})`) + '\n',
    )
