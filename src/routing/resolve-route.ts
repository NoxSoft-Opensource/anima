...
function listAgents(cfg: AnimaConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: AnimaConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: AnimaConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<AnimaConfig, EvaluatedBindingsCache>();
