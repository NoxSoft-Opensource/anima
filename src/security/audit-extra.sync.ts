...
function summarizeGroupPolicy(cfg: AnimaConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: AnimaConfig): boolean {
...
function collectModels(cfg: AnimaConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function listKnownNodeCommands(cfg: AnimaConfig): Set<string> {
  const baseCfg: AnimaConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: AnimaConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: AnimaConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: AnimaConfig): boolean {
...
function isBrowserEnabled(cfg: AnimaConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: AnimaConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: AnimaConfig): SecurityAuditFinding[] {
...
export function collectSecretsInConfigFindings(cfg: AnimaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer ANIMA_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const animaGatewayToken =
    typeof env.ANIMA_GATEWAY_TOKEN === "string" && env.ANIMA_GATEWAY_TOKEN.trim()
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-anima-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: AnimaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectNodeDenyCommandPatternFindings(cfg: AnimaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: AnimaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: AnimaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: AnimaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
