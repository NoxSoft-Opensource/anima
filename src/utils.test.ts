...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "anima-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "anima-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "anima-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "anima-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.anima when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "anima-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers ANIMA_HOME over HOME", () => {
    vi.stubEnv("ANIMA_HOME", "/srv/anima-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/anima-home"));
...
describe("shortenHomePath", () => {
  it("uses $ANIMA_HOME prefix when ANIMA_HOME is set", () => {
    vi.stubEnv("ANIMA_HOME", "/srv/anima-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/anima-home")}/.anima/anima.json`)).toBe(
      "$ANIMA_HOME/.anima/anima.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $ANIMA_HOME replacement when ANIMA_HOME is set", () => {
    vi.stubEnv("ANIMA_HOME", "/srv/anima-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/anima-home")}/.anima/anima.json`),
    ).toBe("config: $ANIMA_HOME/.anima/anima.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/anima")).toBe(path.resolve(os.homedir(), "anima"));
  });
  ...
  it("prefers ANIMA_HOME for tilde expansion", () => {
    vi.stubEnv("ANIMA_HOME", "/srv/anima-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/anima")).toBe(path.resolve("/srv/anima-home", "anima"));
