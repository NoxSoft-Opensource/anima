...
describe("setupOnboardingShellCompletion", () => {
  it("QuickStart: installs without prompting", async () => {
    ...
    const deps = {
      resolveCliName: () => "anima",
      checkShellCompletionStatus: vi.fn(async () => ({
        shell: "zsh",
        profileInstalled: false,
        cacheExists: false,
        cachePath: "/tmp/anima.zsh",
    ...
    expect(deps.ensureCompletionCacheExists).toHaveBeenCalledWith("anima");
    expect(deps.installCompletion).toHaveBeenCalledWith("zsh", true, "anima");
