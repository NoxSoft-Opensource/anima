...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    animaHome: process.env.ANIMA_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("ANIMA_HOME", snapshot.animaHome);
  restoreKey("ANIMA_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked ANIMA_HOME.
  delete process.env.ANIMA_HOME;
  process.env.ANIMA_STATE_DIR = path.join(base, ".anima");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "anima-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".anima", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
