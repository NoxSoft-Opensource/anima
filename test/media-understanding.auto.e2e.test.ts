...
import type { AnimaConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("anima-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("anima-bin-sherpa-");
      const modelDir = await makeTempDir("anima-sherpa-model-");
      ...
      const cfg: AnimaConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("anima-bin-whispercpp-");
      const modelDir = await makeTempDir("anima-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("anima-bin-gemini-");
      ...
      const cfg: AnimaConfig = { tools: { media: { image: {} } } };
