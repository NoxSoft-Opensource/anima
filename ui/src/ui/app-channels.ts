...
export async function handleWhatsAppStart(host: AnimaApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: AnimaApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: AnimaApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: AnimaApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: AnimaApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: AnimaApp): string {
...
function resolveGatewayHttpAuthHeader(host: AnimaApp): string | null {
...
function buildGatewayHttpHeaders(host: AnimaApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: AnimaApp) {
...
export function handleNostrProfileToggleAdvanced(host: AnimaApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: AnimaApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: AnimaApp) {
  const state = host.nostrProfileFormState;
