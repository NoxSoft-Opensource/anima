...
const originalConnect = AnimaApp.prototype.connect;
...
afterEach(() => {
  AnimaApp.prototype.connect = originalConnect;
  window.__ANIMA_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/anima/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/anima");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/anima/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__ANIMA_CONTROL_UI_BASE_PATH__ = "/anima";
    const app = mountApp("/anima/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/anima");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/anima/sessions");
  });
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem("anima.control.settings.v1", JSON.stringify({ token: "existing-token" }));
    const app = mountApp("/ui/overview?token=abc123");
    await app.updateComplete;
